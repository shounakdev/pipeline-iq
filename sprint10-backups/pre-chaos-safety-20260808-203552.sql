...
\restrict ucBaALHpKHfNnidNp5lwdjHNe87b0S9pQC8gDBhpymzPS7ZOzAYfvtQvhQ0YSDw
...
--
-- Name: benchmark_status; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.benchmark_status AS ENUM (
    'PASSED',
    'FAILED',
    'INCOMPLETE'
);
...
ALTER TYPE public.benchmark_status OWNER TO postgres;

--
-- Name: chaos_observation_type; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.chaos_observation_type AS ENUM (
    'FAILURE_INJECTED',
    'TELEMETRY_ANOMALY',
    'ALERT_CREATED',
    'INCIDENT_CREATED',
...
ALTER TYPE public.chaos_observation_type OWNER TO postgres;

--
-- Name: chaos_run_status; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.chaos_run_status AS ENUM (
    'PENDING',
    'RUNNING',
    'FAULT_INJECTED',
    'OBSERVING',
...
ALTER TYPE public.chaos_run_status OWNER TO postgres;

--
-- Name: chaos_scenario_type; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.chaos_scenario_type AS ENUM (
    'FAULTY_RELEASE',
    'POD_KILL',
    'NETWORK_DELAY',
    'DATABASE_DELAY',
...
ALTER TYPE public.chaos_scenario_type OWNER TO postgres;

--
-- Name: diagnosis_rating; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.diagnosis_rating AS ENUM (
    'CORRECT',
    'PARTIALLY_CORRECT',
    'INCORRECT',
    'NOT_AVAILABLE'
...
ALTER TYPE public.diagnosis_rating OWNER TO postgres;
...
--
-- Name: evidence_collection_status; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.evidence_collection_status AS ENUM (
    'PENDING',
    'COLLECTING',
    'COMPLETED',
    'PARTIAL',
...
ALTER TYPE public.evidence_collection_status OWNER TO postgres;
...
--
-- Name: rca_confidence; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.rca_confidence AS ENUM (
    'LOW',
    'MEDIUM',
    'HIGH'
);
...
ALTER TYPE public.rca_confidence OWNER TO postgres;

--
-- Name: rca_feedback_rating; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.rca_feedback_rating AS ENUM (
    'CORRECT',
    'PARTIALLY_CORRECT',
    'INCORRECT'
);
...
ALTER TYPE public.rca_feedback_rating OWNER TO postgres;

--
-- Name: rca_report_status; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.rca_report_status AS ENUM (
    'PENDING',
    'GENERATING',
    'COMPLETED',
    'FAILED'
...
ALTER TYPE public.rca_report_status OWNER TO postgres;

--
-- Name: recommendation_status; Type: TYPE; Schema: public; Owner: postgres
--
...
--
-- Name: recovery_verification_status; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.recovery_verification_status AS ENUM (
    'PENDING',
    'VERIFIED',
    'FAILED'
);
...
ALTER TYPE public.recovery_verification_status OWNER TO postgres;
...
--
-- Name: remediation_action_type; Type: TYPE; Schema: public; Owner: postgres
--
...
ALTER TYPE public.remediation_action_type OWNER TO postgres;

--
-- Name: remediation_approval_decision; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.remediation_approval_decision AS ENUM (
    'APPROVED',
    'REJECTED'
);


ALTER TYPE public.remediation_approval_decision OWNER TO postgres;

--
-- Name: remediation_execution_status; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.remediation_execution_status AS ENUM (
    'PENDING',
    'IN_PROGRESS',
    'SUCCEEDED',
    'FAILED'
...
ALTER TYPE public.remediation_execution_status OWNER TO postgres;
...
--
-- Name: next_incident_number(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.next_incident_number() RETURNS character varying
    LANGUAGE plpgsql
    AS $$
        BEGIN
            RETURN 'INC-' ||
...
ALTER FUNCTION public.next_incident_number() OWNER TO postgres;
...
CREATE TABLE public.chaos_experiments (
    ...
    description text,
    scenario_type public.chaos_scenario_type NOT NULL,
    target_service_id character varying(36) NOT NULL,
    target_environment character varying(100) NOT NULL,
    target_namespace character varying(255) NOT NULL,
    failure_type character varying(100) NOT NULL,
    failure_config jsonb DEFAULT '{}'::jsonb NOT NULL,
...
--
-- Name: chaos_observations; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.chaos_observations (
    id uuid NOT NULL,
    chaos_run_id uuid NOT NULL,
    observation_type public.chaos_observation_type NOT NULL,
...
ALTER TABLE public.chaos_observations OWNER TO postgres;

--
-- Name: chaos_runs; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.chaos_runs (
    id uuid NOT NULL,
    experiment_id uuid NOT NULL,
    status public.chaos_run_status DEFAULT 'PENDING'::public.chaos_run_status NOT NULL,
    ...
    CONSTRAINT ck_chaos_run_aborted_after_started CHECK (((aborted_at IS NULL) OR (started_at IS NULL) OR (aborted_at >= started_at))),
    CONSTRAINT ck_chaos_run_completed_after_started CHECK (((completed_at IS NULL) OR (started_at IS NULL) OR (completed_at >= started_at)))
);


ALTER TABLE public.chaos_runs OWNER TO postgres;
...
--
-- Name: experiment_benchmarks; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.experiment_benchmarks (
    id uuid NOT NULL,
    chaos_run_id uuid NOT NULL,
    failure_injection_timestamp timestamp with time zone,
    first_anomaly_timestamp timestamp with time zone,
    alert_creation_timestamp timestamp with time zone,
    incident_creation_timestamp timestamp with time zone,
    rca_completion_timestamp timestamp with time zone,
    remediation_approval_timestamp timestamp with time zone,
    recovery_completion_timestamp timestamp with time zone,
    time_to_detect_ms integer,
    time_to_alert_ms integer,
    time_to_incident_ms integer,
    time_to_diagnose_ms integer,
    time_to_approve_ms integer,
    time_to_recover_ms integer,
    diagnosis_rating public.diagnosis_rating DEFAULT 'NOT_AVAILABLE'::public.diagnosis_rating NOT NULL,
    expected_root_cause text,
    actual_root_cause text,
    detection_succeeded boolean,
    recovery_succeeded boolean,
    benchmark_status public.benchmark_status DEFAULT 'INCOMPLETE'::public.benchmark_status NOT NULL,
    calculated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_experiment_benchmark_alert_nonnegative CHECK (((time_to_alert_ms IS NULL) OR (time_to_alert_ms >= 0))),
    CONSTRAINT ck_experiment_benchmark_approve_nonnegative CHECK (((time_to_approve_ms IS NULL) OR (time_to_approve_ms >= 0))),
    CONSTRAINT ck_experiment_benchmark_detect_nonnegative CHECK (((time_to_detect_ms IS NULL) OR (time_to_detect_ms >= 0))),
    CONSTRAINT ck_experiment_benchmark_diagnose_nonnegative CHECK (((time_to_diagnose_ms IS NULL) OR (time_to_diagnose_ms >= 0))),
    CONSTRAINT ck_experiment_benchmark_incident_nonnegative CHECK (((time_to_incident_ms IS NULL) OR (time_to_incident_ms >= 0))),
    CONSTRAINT ck_experiment_benchmark_recover_nonnegative CHECK (((time_to_recover_ms IS NULL) OR (time_to_recover_ms >= 0)))
);


ALTER TABLE public.experiment_benchmarks OWNER TO postgres;
...
--
-- Name: incident_events; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.incident_events AS
 SELECT id,
    incident_id,
    event_type,
    message,
    ...
    created_at
   FROM public.incident_timeline_events;


ALTER VIEW public.incident_events OWNER TO postgres;
...
CREATE TABLE public.incident_evidence (
    ...
    status public.evidence_collection_status DEFAULT 'PENDING'::public.evidence_collection_status NOT NULL,
...
CREATE TABLE public.incidents (
    ...
    description text,
    severity public.incidentseverity DEFAULT 'SEV-3'::public.incidentseverity NOT NULL,
    status public.incidentstatus DEFAULT 'DETECTED'::public.incidentstatus NOT NULL,
    service_id character varying,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    incident_number character varying(32) DEFAULT public.next_incident_number() NOT NULL,
    primary_service_id character varying(36),
    triggering_alert_id character varying(36),
    suspected_deployment_id uuid,
    deduplication_key character varying(500),
    ...
    current_assignee_id character varying(36),
...
--
-- Name: rca_feedback; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.rca_feedback (
    id uuid NOT NULL,
    incident_id uuid NOT NULL,
    report_id uuid NOT NULL,
    rating public.rca_feedback_rating NOT NULL,
    comment text,
...
ALTER TABLE public.rca_feedback OWNER TO postgres;
...
CREATE TABLE public.rca_reports (
    ...
    version integer NOT NULL,
    status public.rca_report_status DEFAULT 'PENDING'::public.rca_report_status NOT NULL,
    schema_version character varying(20) DEFAULT '1.0'::character varying NOT NULL,
    confidence public.rca_confidence,
    summary text,
...
--
-- Name: recovery_verifications; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.recovery_verifications (
    id uuid NOT NULL,
    remediation_id uuid NOT NULL,
    remediation_execution_id uuid NOT NULL,
    verification_status public.recovery_verification_status DEFAULT 'PENDING'::public.recovery_verification_status NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_recovery_verification_verified_at CHECK (((verification_status = 'PENDING'::public.recovery_verification_status) OR (verified_at IS NOT NULL)))
);


ALTER TABLE public.recovery_verifications OWNER TO postgres;
...
--
-- Name: remediation_approvals; Type: TABLE; Schema: public; Owner: postgres
--
...
CREATE TABLE public.remediation_approvals (
    ...
    decision public.remediation_approval_decision NOT NULL,
    rejection_reason text,
    approved_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_remediation_approval_rejection_reason CHECK (((decision <> 'REJECTED'::public.remediation_approval_decision) OR ((rejection_reason IS NOT NULL) AND (length(TRIM(BOTH FROM rejection_reason)) > 0))))
);


ALTER TABLE public.remediation_approvals OWNER TO postgres;

--
-- Name: remediation_executions; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.remediation_executions (
    id uuid NOT NULL,
    remediation_id uuid NOT NULL,
    command_type public.remediation_action_type NOT NULL,
    command_payload jsonb NOT NULL,
    execution_status public.remediation_execution_status DEFAULT 'PENDING'::public.remediation_execution_status NOT NULL,
    started_at timestamp with time zone,
    completed_at timestamp with time zone,
    result_summary jsonb NOT NULL,
    error_message text,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_remediation_execution_timestamp_order CHECK (((completed_at IS NULL) OR (started_at IS NULL) OR (completed_at >= started_at)))
);


ALTER TABLE public.remediation_executions OWNER TO postgres;

--
-- Name: remediation_recommendations; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.remediation_recommendations (
    id uuid NOT NULL,
    incident_id uuid NOT NULL,
    service_id character varying(36) NOT NULL,
    ...
    confidence public.rca_confidence NOT NULL,
    status public.recommendation_status DEFAULT 'PENDING_APPROVAL'::public.recommendation_status NOT NULL,
    created_by character varying(36),
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL
);
...
ALTER TABLE public.remediation_recommendations OWNER TO postgres;
...
396b93ca-c2c8-4618-bcb8-4f9cc85db101	\N	INCIDENT_CREATED	INCIDENT	da71ef13-66cc-40fd-a81e-f30aa46388d6	{"alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "deduplication_key": "79a28492-b0d4-4098-94ce-70c4df2785cd:staging:error_budget_exhausted", "environment": "staging", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "severity": "SEV-2"}	2026-07-16 19:31:20.724453
9189a510-d413-497e-aa11-b6c858d47e13	8710aab9-161d-41e4-8149-d8c00772d426	INCIDENT_ASSIGNED	Incident	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	{"incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "previous_assignee_user_id": null, "assigned_to_user_id": "8710aab9-161d-41e4-8149-d8c00772d426", "assigned_to_user_email": "hello@123.admin.com", "assignment_note": "done", "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/assign"}	2026-07-19 20:08:24.09187
68f1be59-654e-4085-93c9-4812a0b926f8	8710aab9-161d-41e4-8149-d8c00772d426	INCIDENT_ASSIGNED	Incident	f0fff6a0-aa8e-4bd4-be53-4af20d913a8d	{"incident_id": "f0fff6a0-aa8e-4bd4-be53-4af20d913a8d", "incident_number": "INC-006", "previous_assignee_user_id": null, "assigned_to_user_id": "8710aab9-161d-41e4-8149-d8c00772d426", "assigned_to_user_email": "hello@123.admin.com", "assignment_note": "Hello checking it 123", "method": "POST", "request_path": "/api/incidents/f0fff6a0-aa8e-4bd4-be53-4af20d913a8d/assign"}	2026-07-19 20:09:12.126763
ed62c1a1-87de-41c5-ac39-646ccd22666e	8710aab9-161d-41e4-8149-d8c00772d426	INCIDENT_COMMENT_CREATED	IncidentComment	814af5e1-da59-4d78-ad03-fef289dbb5ca	{"comment_id": "814af5e1-da59-4d78-ad03-fef289dbb5ca", "comment_length": 10, "incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/comments"}	2026-07-19 20:14:35.92732
175a3b0a-2654-48a7-95d5-3ab2e0bdfab6	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	INCIDENT_ACKNOWLEDGED	Incident	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	{"incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "from_status": "DETECTED", "to_status": "ACKNOWLEDGED", "reason": "Acknowledging it", "resolution_summary": null, "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/acknowledge", "assign_to_self": true, "assigned_to_user_id": "ec2c769f-e1d2-42e4-a0af-0cab146e19bd"}	2026-08-02 09:48:25.91004
bd79a336-2bf6-4a30-b9ab-f97a98d252ec	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	INCIDENT_STATUS_CHANGED	Incident	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	{"incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "from_status": "ACKNOWLEDGED", "to_status": "INVESTIGATING", "reason": "Chanign it", "resolution_summary": null, "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/status"}	2026-08-02 09:48:36.52861
6bc3419f-9fb7-49ff-b067-57df1ddbb852	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	INCIDENT_STATUS_CHANGED	Incident	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	{"incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "from_status": "INVESTIGATING", "to_status": "ACTION_RECOMMENDED", "reason": "abc", "resolution_summary": null, "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/status"}	2026-08-02 09:48:50.451606
87dd90ef-ca12-49ef-b388-2eedb0079097	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	INCIDENT_STATUS_CHANGED	Incident	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	{"incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "from_status": "ACTION_RECOMMENDED", "to_status": "REMEDIATING", "reason": "d", "resolution_summary": null, "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/status"}	2026-08-02 09:48:55.022837
1b22a1d4-64c3-4432-a953-63797bffbd56	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	INCIDENT_ASSIGNED	Incident	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	{"incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "incident_number": "INC-008", "previous_assignee_user_id": "ec2c769f-e1d2-42e4-a0af-0cab146e19bd", "assigned_to_user_id": "8710aab9-161d-41e4-8149-d8c00772d426", "assigned_to_user_email": "hello@123.admin.com", "assignment_note": null, "method": "POST", "request_path": "/api/incidents/c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b/assign"}	2026-08-02 09:48:58.431995
8a32ed4b-9904-4aeb-91ce-900dde3311a7	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	REMEDIATION_RECOMMENDED	RemediationRecommendation	ad63d41f-a383-4bcf-941f-c2c87e3e8dff	{"action_type": "ROLLBACK_DEPLOYMENT", "actor_type": "SYSTEM", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "requested_by": "ec2c769f-e1d2-42e4-a0af-0cab146e19bd", "requires_approval": true, "rule_code": "RECENT_DEPLOYMENT_REGRESSION"}	2026-08-02 10:08:15.041344
3a5f0b57-5998-4235-a032-91cb8539d364	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	REMEDIATION_APPROVED	RemediationRecommendation	ad63d41f-a383-4bcf-941f-c2c87e3e8dff	{"action_type": "ROLLBACK_DEPLOYMENT", "actor_type": "HUMAN", "approval_id": "3628440e-e946-4c08-816d-6c44514db7c5", "decision": "APPROVED", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "new_status": "APPROVED", "previous_status": "PENDING_APPROVAL", "rejection_reason": null, "service_id": "b350a986-e527-4825-9efb-99313d9abf09"}	2026-08-02 10:11:39.499768
\.
...
COPY public.chaos_experiments (id, name, description, scenario_type, target_service_id, target_environment, target_namespace, failure_type, failure_config, expected_behavior, enabled, created_by, created_at, updated_at) FROM stdin;
\.


--
-- Data for Name: chaos_observations; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.chaos_observations (id, chaos_run_id, observation_type, source, observed_at, resource_type, resource_id, details, created_at) FROM stdin;
\.


--
-- Data for Name: chaos_runs; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.chaos_runs (id, experiment_id, status, triggered_by, started_at, failure_injected_at, completed_at, aborted_at, failure_message, incident_id, rca_report_id, remediation_id, remediation_execution_id, recovery_verification_id) FROM stdin;
\.
...
686d38f6-856e-400e-b41e-c54cca37f39f	evt_d2f2c22b-8fc5-42b2-be0e-b64ab3d6f1e6	RELIABILITY_ALERT_CREATED	sprint-7d-failure-test	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	{"source": "sprint-7d-failure-test", "alert_id": "missing-alert-7d-test", "severity": "HIGH", "alert_type": "LATENCY_BREACH", "threshold_value": 500.0, "triggered_value": 2300.0, "reliability_alert_id": "missing-alert-7d-test"}	0	OPEN	2026-07-16 19:36:20.229821+00	\N	telemetry.alerts	{"event_id": "evt_d2f2c22b-8fc5-42b2-be0e-b64ab3d6f1e6", "event_type": "RELIABILITY_ALERT_CREATED", "schema_version": "1.0", "correlation_id": "sprint-7d-failure-test", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "timestamp": "2026-07-16T19:36:11.158417+00:00", "payload": {"source": "sprint-7d-failure-test", "alert_id": "missing-alert-7d-test", "severity": "HIGH", "alert_type": "LATENCY_BREACH", "threshold_value": 500.0, "triggered_value": 2300.0, "reliability_alert_id": "missing-alert-7d-test"}}	ReliabilityAlert not found: alert_id=missing-alert-7d-test, event_id=evt_d2f2c22b-8fc5-42b2-be0e-b64ab3d6f1e6	2026-07-16 19:36:20.229821+00
\.
...
47dec4a9-59e4-407a-bc68-94ef45171c98	evt_8bb2a21c-f1ea-42fc-9fd3-550e22fd6869	RELIABILITY_ALERT_CREATED	telemetry.alerts	1.0	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:ERROR_BUDGET_EXHAUSTED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	{"source": "sprint-7d-e2e-test", "alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "severity": "HIGH", "alert_type": "ERROR_BUDGET_EXHAUSTED", "created_at": "2026-07-11T21:10:18.324058+00:00", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "threshold_value": 99.9, "triggered_value": 99.5, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}	PROCESSED	2026-07-16 19:31:20.787583+00	2026-07-16 19:31:20.479881+00	2026-07-16 19:31:15.654431+00	{"event_id": "evt_8bb2a21c-f1ea-42fc-9fd3-550e22fd6869", "event_type": "RELIABILITY_ALERT_CREATED", "schema_version": "1.0", "correlation_id": "79a28492-b0d4-4098-94ce-70c4df2785cd:staging:ERROR_BUDGET_EXHAUSTED", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "timestamp": "2026-07-16T19:31:15.654431+00:00", "payload": {"source": "sprint-7d-e2e-test", "alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "severity": "HIGH", "alert_type": "ERROR_BUDGET_EXHAUSTED", "created_at": "2026-07-11T21:10:18.324058+00:00", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "threshold_value": 99.9, "triggered_value": 99.5, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}}	\N
7a91628e-0c5c-421d-bdef-a03c577adc88	evt_fce541a1-eb72-416a-a42d-51b131ea7adf	RELIABILITY_ALERT_CREATED	telemetry.alerts	1.0	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:ERROR_BUDGET_EXHAUSTED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	{"source": "sprint-7d-duplicate-test", "alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "severity": "HIGH", "alert_type": "ERROR_BUDGET_EXHAUSTED", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "threshold_value": 99.9, "triggered_value": 99.5, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}	PROCESSED	2026-07-16 19:34:29.433774+00	2026-07-16 19:34:29.256927+00	2026-07-16 19:34:21.848092+00	{"event_id": "evt_fce541a1-eb72-416a-a42d-51b131ea7adf", "event_type": "RELIABILITY_ALERT_CREATED", "schema_version": "1.0", "correlation_id": "79a28492-b0d4-4098-94ce-70c4df2785cd:staging:ERROR_BUDGET_EXHAUSTED", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "timestamp": "2026-07-16T19:34:21.848092+00:00", "payload": {"source": "sprint-7d-duplicate-test", "alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "severity": "HIGH", "alert_type": "ERROR_BUDGET_EXHAUSTED", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "threshold_value": 99.9, "triggered_value": 99.5, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}}	\N
\.


--
-- Data for Name: experiment_benchmarks; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.experiment_benchmarks (id, chaos_run_id, failure_injection_timestamp, first_anomaly_timestamp, alert_creation_timestamp, incident_creation_timestamp, rca_completion_timestamp, remediation_approval_timestamp, recovery_completion_timestamp, time_to_detect_ms, time_to_alert_ms, time_to_incident_ms, time_to_diagnose_ms, time_to_approve_ms, time_to_recover_ms, diagnosis_rating, expected_root_cause, actual_root_cause, detection_succeeded, recovery_succeeded, benchmark_status, calculated_at) FROM stdin;
\.
...
55ee0c80-8f53-4281-98f2-ea3ddb60e6ce	da71ef13-66cc-40fd-a81e-f30aa46388d6	d0391e45-448f-45b0-b978-be94302328b7	2026-07-16 19:31:20.479881+00	t
\.
...
1c7b83c2-529c-4901-a5b2-347e860d0484	f0fff6a0-aa8e-4bd4-be53-4af20d913a8d	8710aab9-161d-41e4-8149-d8c00772d426	8710aab9-161d-41e4-8149-d8c00772d426	Hello checking it 123	2026-07-19 20:09:12.123016+00	\N	t
79f308c4-4200-4219-b21b-08da0eb92608	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	8710aab9-161d-41e4-8149-d8c00772d426	8710aab9-161d-41e4-8149-d8c00772d426	done	2026-07-19 20:08:24.031253+00	2026-08-02 09:48:25.901815+00	f
f12e7876-8ab6-4061-8b39-24ee2a18295a	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	Acknowledging it	2026-08-02 09:48:25.901815+00	2026-08-02 09:48:58.427571+00	f
ca62412e-e644-4a02-b37f-3178b6bf0f8c	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	8710aab9-161d-41e4-8149-d8c00772d426	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	\N	2026-08-02 09:48:58.427571+00	\N	t
\.
...
814af5e1-da59-4d78-ad03-fef289dbb5ca	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	8710aab9-161d-41e4-8149-d8c00772d426	Lets check	2026-07-19 20:14:35.886668+00	2026-07-19 20:14:35.885018+00
\.
...
COPY public.incident_evidence (id, incident_id, version, status, schema_version, window_start, window_end, anchor_time, evidence_payload, source_statuses, collection_errors, created_by, created_at, updated_at) FROM stdin;
b6716bb2-78e2-468b-a65b-0e31526377be	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	1	PENDING	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 10:41:33.164082+00	2026-07-26 10:41:33.164082+00
1bf76375-6f58-429f-b750-70b5abb985b2	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	2	PARTIAL	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 10:50:52.837621+00	2026-07-26 10:50:53.032306+00
fdbb7887-7cbf-48a6-a586-4c0d53b995a7	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	3	FAILED	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 10:57:50.214315+00	2026-07-26 10:57:50.874495+00
c464bf88-363e-43da-afea-01b51f0e4d21	da71ef13-66cc-40fd-a81e-f30aa46388d6	1	FAILED	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 10:58:36.748593+00	2026-07-26 10:58:36.777979+00
7a94d9b1-0fea-4798-b30f-75b49717bbc3	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	4	FAILED	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 11:01:56.138825+00	2026-07-26 11:01:56.320868+00
86df0962-45f3-46a6-a91b-cb7b862161b7	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	5	PARTIAL	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 11:04:42.605742+00	2026-07-26 11:04:42.773018+00
950dcdd1-8f15-4040-9bfd-20e9c21e625d	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	6	PARTIAL	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 11:09:16.963854+00	2026-07-26 11:09:17.265717+00
856a0caf-613e-42ee-af77-be3213bf2938	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	7	PARTIAL	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 11:25:30.247565+00	2026-07-26 11:25:30.64284+00
52a4820d-f857-44ce-8fa0-eac9581e0344	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	8	PARTIAL	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 11:32:44.486029+00	2026-07-26 11:32:44.814398+00
1eb1c41a-45f4-4f98-aae8-52a959acd1c0	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	9	PARTIAL	v1	\N	\N	\N	{}	{}	[]	\N	2026-07-26 11:33:33.34458+00	2026-07-26 11:33:33.358625+00
b184e6bb-91df-419d-b058-5b81420f91b0	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	10	PARTIAL	v1	\N	\N	\N	{"slo": {"status": "COLLECTED", "target": null, "slo_type": null, "burn_rate": null, "created_at": "2026-07-11T21:10:18.324058+00:00", "breach_status": null, "alert_severity": "HIGH", "measured_value": null, "window_minutes": null, "breach_started_at": null, "slo_definition_id": "4b572d35-cb52-47f2-9b91-f98577ade93a", "error_budget_status": null, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}, "logs": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_error_logs": 0, "error_count_after": 0, "error_count_before": 0, "out_of_memory_errors": 0, "top_error_signatures": [], "first_error_timestamp": null, "database_timeout_errors": 0, "connection_refused_errors": 0, "uncaught_exception_errors": 0}, "status": "PARTIAL", "traces": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_traces": 0, "top_failed_span": null, "slow_trace_count": 0, "failed_trace_count": 0, "top_failed_dependency": null, "dependency_error_counts": {}, "failed_trace_percentage": 0, "p95_trace_duration_after": null, "representative_trace_ids": [], "p95_trace_duration_before": null, "p95_trace_duration_change": null}, "metrics": {"status": "PARTIAL", "queries": {"cpu": "container_cpu_usage", "memory": "container_memory_usage", "error_rate": "service_error_rate", "request_rate": "http_request_rate", "http_500_rate": "http_500_rate", "p95_latency_ms": "service_p95_latency_ms", "available_replicas": "kube_deployment_available_replicas"}, "service": null, "cpu_after": null, "cpu_before": null, "memory_after": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_before": null, "missing_series": ["request_rate", "http_500_rate", "error_rate", "p95_latency_ms", "cpu", "memory", "available_replicas"], "error_rate_after": null, "error_rate_before": null, "error_rate_change": null, "request_rate_after": null, "http_500_rate_after": null, "request_rate_before": null, "http_500_rate_before": null, "http_500_rate_change": null, "p95_latency_after_ms": null, "p95_latency_before_ms": null, "p95_latency_change_ms": null, "p95_latency_multiplier": null, "available_replicas_after": null, "available_replicas_before": null}, "incident": {"title": "demo-service error budget exhausted in staging", "status": "COLLECTED", "metrics": [], "severity": "SEV-2", "detected_at": "2026-07-11T21:29:10.350774+00:00", "environment": "staging", "incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "resolved_at": null, "acknowledged_at": "2026-08-02T09:48:25.901815+00:00", "incident_number": "INC-008", "incident_status": "REMEDIATING", "timeline_events": [{"source": "SYSTEM", "summary": null, "event_type": "INCIDENT_CREATED", "occurred_at": "2026-07-11T21:29:10.350774+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-07-19T20:08:24.031253+00:00"}, {"source": "USER", "summary": null, "event_type": "INCIDENT_COMMENT_ADDED", "occurred_at": "2026-07-19T20:14:35.886668+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACKNOWLEDGED", "occurred_at": "2026-08-02T09:48:25.901815+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_INVESTIGATING", "occurred_at": "2026-08-02T09:48:36.524654+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACTION_RECOMMENDED", "occurred_at": "2026-08-02T09:48:50.449715+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_REMEDIATING", "occurred_at": "2026-08-02T09:48:55.019151+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-08-02T09:48:58.427571+00:00"}], "related_alert_ids": [], "failure_started_at": null, "primary_service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "human_context_included": false, "suspected_deployment_id": null}, "pipeline": {"reason": "No pipeline_run_id available from deployment evidence", "status": "NO_DATA"}, "deployment": {"reason": "Missing incident failure time or service for deployment correlation", "status": "NO_DATA", "correlation_method": "NO_DATA"}, "kubernetes": {"status": "COLLECTED", "cpu_status": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_status": null, "restart_delta": null, "crash_loop_count": null, "desired_replicas": null, "oom_killed_count": null, "pending_pod_count": null, "pod_restart_count": null, "available_replicas": null, "unavailable_replicas": null, "recent_warning_events": [], "failed_liveness_probe_count": null, "failed_readiness_probe_count": null}, "derived_facts": [{"fact_type": "SLO_BREACH", "description": "A reliability alert indicates the measured value breached the configured threshold.", "evidence_paths": ["slo.measured_value", "slo.target", "slo.alert_severity"]}], "evidence_hash": "32872f3d6c5cf6c4842cb64cf5ccdb6734c732194e5bc8624826a068467e4c07", "schema_version": "rca-evidence-bundle/v1", "missing_sources": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "collector_errors": [], "correlation_facts": [{"polarity": "CONTRADICTORY", "fact_type": "NO_POD_HEALTH_DEGRADATION", "description": "No pod restarts or readiness failures occurred.", "evidence_paths": ["kubernetes.pod_restart_count", "kubernetes.failed_readiness_probe_count"]}], "completeness_score": 0.75}	{"slo": "COLLECTED", "logs": "COLLECTED", "traces": "COLLECTED", "metrics": "PARTIAL", "pipeline": "NO_DATA", "deployment": "NO_DATA", "kubernetes": "COLLECTED"}	[]	\N	2026-08-02 09:47:49.607921+00	2026-08-02 09:56:17.643567+00
d77cd1a6-683d-4eb7-b646-f1c659485844	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	12	PARTIAL	v1	\N	\N	\N	{"slo": {"status": "COLLECTED", "target": null, "slo_type": null, "burn_rate": null, "created_at": "2026-07-11T21:10:18.324058+00:00", "breach_status": null, "alert_severity": "HIGH", "measured_value": null, "window_minutes": null, "breach_started_at": null, "slo_definition_id": "4b572d35-cb52-47f2-9b91-f98577ade93a", "error_budget_status": null, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}, "logs": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_error_logs": 0, "error_count_after": 0, "error_count_before": 0, "out_of_memory_errors": 0, "top_error_signatures": [], "first_error_timestamp": null, "database_timeout_errors": 0, "connection_refused_errors": 0, "uncaught_exception_errors": 0}, "status": "PARTIAL", "traces": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_traces": 0, "top_failed_span": null, "slow_trace_count": 0, "failed_trace_count": 0, "top_failed_dependency": null, "dependency_error_counts": {}, "failed_trace_percentage": 0, "p95_trace_duration_after": null, "representative_trace_ids": [], "p95_trace_duration_before": null, "p95_trace_duration_change": null}, "metrics": {"status": "PARTIAL", "queries": {"cpu": "container_cpu_usage", "memory": "container_memory_usage", "error_rate": "service_error_rate", "request_rate": "http_request_rate", "http_500_rate": "http_500_rate", "p95_latency_ms": "service_p95_latency_ms", "available_replicas": "kube_deployment_available_replicas"}, "service": null, "cpu_after": null, "cpu_before": null, "memory_after": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_before": null, "missing_series": ["request_rate", "http_500_rate", "error_rate", "p95_latency_ms", "cpu", "memory", "available_replicas"], "error_rate_after": null, "error_rate_before": null, "error_rate_change": null, "request_rate_after": null, "http_500_rate_after": null, "request_rate_before": null, "http_500_rate_before": null, "http_500_rate_change": null, "p95_latency_after_ms": null, "p95_latency_before_ms": null, "p95_latency_change_ms": null, "p95_latency_multiplier": null, "available_replicas_after": null, "available_replicas_before": null}, "incident": {"title": "demo-service error budget exhausted in staging", "status": "COLLECTED", "metrics": [], "severity": "SEV-2", "detected_at": "2026-07-11T21:29:10.350774+00:00", "environment": "staging", "incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "resolved_at": null, "acknowledged_at": "2026-08-02T09:48:25.901815+00:00", "incident_number": "INC-008", "incident_status": "REMEDIATING", "timeline_events": [{"source": "SYSTEM", "summary": null, "event_type": "INCIDENT_CREATED", "occurred_at": "2026-07-11T21:29:10.350774+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-07-19T20:08:24.031253+00:00"}, {"source": "USER", "summary": null, "event_type": "INCIDENT_COMMENT_ADDED", "occurred_at": "2026-07-19T20:14:35.886668+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACKNOWLEDGED", "occurred_at": "2026-08-02T09:48:25.901815+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_INVESTIGATING", "occurred_at": "2026-08-02T09:48:36.524654+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACTION_RECOMMENDED", "occurred_at": "2026-08-02T09:48:50.449715+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_REMEDIATING", "occurred_at": "2026-08-02T09:48:55.019151+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-08-02T09:48:58.427571+00:00"}], "related_alert_ids": [], "failure_started_at": null, "primary_service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "human_context_included": false, "suspected_deployment_id": null}, "pipeline": {"reason": "No pipeline_run_id available from deployment evidence", "status": "NO_DATA"}, "deployment": {"reason": "Missing incident failure time or service for deployment correlation", "status": "NO_DATA", "correlation_method": "NO_DATA"}, "kubernetes": {"status": "COLLECTED", "cpu_status": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_status": null, "restart_delta": null, "crash_loop_count": null, "desired_replicas": null, "oom_killed_count": null, "pending_pod_count": null, "pod_restart_count": null, "available_replicas": null, "unavailable_replicas": null, "recent_warning_events": [], "failed_liveness_probe_count": null, "failed_readiness_probe_count": null}, "derived_facts": [{"fact_type": "SLO_BREACH", "description": "A reliability alert indicates the measured value breached the configured threshold.", "evidence_paths": ["slo.measured_value", "slo.target", "slo.alert_severity"]}], "evidence_hash": "32872f3d6c5cf6c4842cb64cf5ccdb6734c732194e5bc8624826a068467e4c07", "schema_version": "rca-evidence-bundle/v1", "missing_sources": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "collector_errors": [], "correlation_facts": [{"polarity": "CONTRADICTORY", "fact_type": "NO_POD_HEALTH_DEGRADATION", "description": "No pod restarts or readiness failures occurred.", "evidence_paths": ["kubernetes.pod_restart_count", "kubernetes.failed_readiness_probe_count"]}], "completeness_score": 0.75}	{"slo": "COLLECTED", "logs": "COLLECTED", "traces": "COLLECTED", "metrics": "PARTIAL", "pipeline": "NO_DATA", "deployment": "NO_DATA", "kubernetes": "COLLECTED"}	[]	\N	2026-08-02 09:49:25.60716+00	2026-08-02 09:56:17.641718+00
81317a1c-ee99-469d-a3d1-f6681a678a6c	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	13	PARTIAL	v1	\N	\N	\N	{"slo": {"status": "COLLECTED", "target": null, "slo_type": null, "burn_rate": null, "created_at": "2026-07-11T21:10:18.324058+00:00", "breach_status": null, "alert_severity": "HIGH", "measured_value": null, "window_minutes": null, "breach_started_at": null, "slo_definition_id": "4b572d35-cb52-47f2-9b91-f98577ade93a", "error_budget_status": null, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}, "logs": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_error_logs": 0, "error_count_after": 0, "error_count_before": 0, "out_of_memory_errors": 0, "top_error_signatures": [], "first_error_timestamp": null, "database_timeout_errors": 0, "connection_refused_errors": 0, "uncaught_exception_errors": 0}, "status": "PARTIAL", "traces": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_traces": 0, "top_failed_span": null, "slow_trace_count": 0, "failed_trace_count": 0, "top_failed_dependency": null, "dependency_error_counts": {}, "failed_trace_percentage": 0, "p95_trace_duration_after": null, "representative_trace_ids": [], "p95_trace_duration_before": null, "p95_trace_duration_change": null}, "metrics": {"status": "PARTIAL", "queries": {"cpu": "container_cpu_usage", "memory": "container_memory_usage", "error_rate": "service_error_rate", "request_rate": "http_request_rate", "http_500_rate": "http_500_rate", "p95_latency_ms": "service_p95_latency_ms", "available_replicas": "kube_deployment_available_replicas"}, "service": null, "cpu_after": null, "cpu_before": null, "memory_after": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_before": null, "missing_series": ["request_rate", "http_500_rate", "error_rate", "p95_latency_ms", "cpu", "memory", "available_replicas"], "error_rate_after": null, "error_rate_before": null, "error_rate_change": null, "request_rate_after": null, "http_500_rate_after": null, "request_rate_before": null, "http_500_rate_before": null, "http_500_rate_change": null, "p95_latency_after_ms": null, "p95_latency_before_ms": null, "p95_latency_change_ms": null, "p95_latency_multiplier": null, "available_replicas_after": null, "available_replicas_before": null}, "incident": {"title": "demo-service error budget exhausted in staging", "status": "COLLECTED", "metrics": [], "severity": "SEV-2", "detected_at": "2026-07-11T21:29:10.350774+00:00", "environment": "staging", "incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "resolved_at": null, "acknowledged_at": "2026-08-02T09:48:25.901815+00:00", "incident_number": "INC-008", "incident_status": "REMEDIATING", "timeline_events": [{"source": "SYSTEM", "summary": null, "event_type": "INCIDENT_CREATED", "occurred_at": "2026-07-11T21:29:10.350774+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-07-19T20:08:24.031253+00:00"}, {"source": "USER", "summary": null, "event_type": "INCIDENT_COMMENT_ADDED", "occurred_at": "2026-07-19T20:14:35.886668+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACKNOWLEDGED", "occurred_at": "2026-08-02T09:48:25.901815+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_INVESTIGATING", "occurred_at": "2026-08-02T09:48:36.524654+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACTION_RECOMMENDED", "occurred_at": "2026-08-02T09:48:50.449715+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_REMEDIATING", "occurred_at": "2026-08-02T09:48:55.019151+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-08-02T09:48:58.427571+00:00"}], "related_alert_ids": [], "failure_started_at": null, "primary_service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "human_context_included": false, "suspected_deployment_id": null}, "pipeline": {"reason": "No pipeline_run_id available from deployment evidence", "status": "NO_DATA"}, "deployment": {"reason": "Missing incident failure time or service for deployment correlation", "status": "NO_DATA", "correlation_method": "NO_DATA"}, "kubernetes": {"status": "COLLECTED", "cpu_status": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_status": null, "restart_delta": null, "crash_loop_count": null, "desired_replicas": null, "oom_killed_count": null, "pending_pod_count": null, "pod_restart_count": null, "available_replicas": null, "unavailable_replicas": null, "recent_warning_events": [], "failed_liveness_probe_count": null, "failed_readiness_probe_count": null}, "derived_facts": [{"fact_type": "SLO_BREACH", "description": "A reliability alert indicates the measured value breached the configured threshold.", "evidence_paths": ["slo.measured_value", "slo.target", "slo.alert_severity"]}], "evidence_hash": "32872f3d6c5cf6c4842cb64cf5ccdb6734c732194e5bc8624826a068467e4c07", "schema_version": "rca-evidence-bundle/v1", "missing_sources": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "collector_errors": [], "correlation_facts": [{"polarity": "CONTRADICTORY", "fact_type": "NO_POD_HEALTH_DEGRADATION", "description": "No pod restarts or readiness failures occurred.", "evidence_paths": ["kubernetes.pod_restart_count", "kubernetes.failed_readiness_probe_count"]}], "completeness_score": 0.75}	{"slo": "COLLECTED", "logs": "COLLECTED", "traces": "COLLECTED", "metrics": "PARTIAL", "pipeline": "NO_DATA", "deployment": "NO_DATA", "kubernetes": "COLLECTED"}	[]	\N	2026-08-02 09:51:58.134619+00	2026-08-02 09:56:17.86835+00
56645ecc-93cc-4a2b-801e-493fd15b28c3	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	11	PARTIAL	v1	\N	\N	\N	{"slo": {"status": "COLLECTED", "target": null, "slo_type": null, "burn_rate": null, "created_at": "2026-07-11T21:10:18.324058+00:00", "breach_status": null, "alert_severity": "HIGH", "measured_value": null, "window_minutes": null, "breach_started_at": null, "slo_definition_id": "4b572d35-cb52-47f2-9b91-f98577ade93a", "error_budget_status": null, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}, "logs": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_error_logs": 0, "error_count_after": 0, "error_count_before": 0, "out_of_memory_errors": 0, "top_error_signatures": [], "first_error_timestamp": null, "database_timeout_errors": 0, "connection_refused_errors": 0, "uncaught_exception_errors": 0}, "status": "PARTIAL", "traces": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_traces": 0, "top_failed_span": null, "slow_trace_count": 0, "failed_trace_count": 0, "top_failed_dependency": null, "dependency_error_counts": {}, "failed_trace_percentage": 0, "p95_trace_duration_after": null, "representative_trace_ids": [], "p95_trace_duration_before": null, "p95_trace_duration_change": null}, "metrics": {"status": "PARTIAL", "queries": {"cpu": "container_cpu_usage", "memory": "container_memory_usage", "error_rate": "service_error_rate", "request_rate": "http_request_rate", "http_500_rate": "http_500_rate", "p95_latency_ms": "service_p95_latency_ms", "available_replicas": "kube_deployment_available_replicas"}, "service": null, "cpu_after": null, "cpu_before": null, "memory_after": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_before": null, "missing_series": ["request_rate", "http_500_rate", "error_rate", "p95_latency_ms", "cpu", "memory", "available_replicas"], "error_rate_after": null, "error_rate_before": null, "error_rate_change": null, "request_rate_after": null, "http_500_rate_after": null, "request_rate_before": null, "http_500_rate_before": null, "http_500_rate_change": null, "p95_latency_after_ms": null, "p95_latency_before_ms": null, "p95_latency_change_ms": null, "p95_latency_multiplier": null, "available_replicas_after": null, "available_replicas_before": null}, "incident": {"title": "demo-service error budget exhausted in staging", "status": "COLLECTED", "metrics": [], "severity": "SEV-2", "detected_at": "2026-07-11T21:29:10.350774+00:00", "environment": "staging", "incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "resolved_at": null, "acknowledged_at": "2026-08-02T09:48:25.901815+00:00", "incident_number": "INC-008", "incident_status": "REMEDIATING", "timeline_events": [{"source": "SYSTEM", "summary": null, "event_type": "INCIDENT_CREATED", "occurred_at": "2026-07-11T21:29:10.350774+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-07-19T20:08:24.031253+00:00"}, {"source": "USER", "summary": null, "event_type": "INCIDENT_COMMENT_ADDED", "occurred_at": "2026-07-19T20:14:35.886668+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACKNOWLEDGED", "occurred_at": "2026-08-02T09:48:25.901815+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_INVESTIGATING", "occurred_at": "2026-08-02T09:48:36.524654+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACTION_RECOMMENDED", "occurred_at": "2026-08-02T09:48:50.449715+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_REMEDIATING", "occurred_at": "2026-08-02T09:48:55.019151+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-08-02T09:48:58.427571+00:00"}], "related_alert_ids": [], "failure_started_at": null, "primary_service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "human_context_included": false, "suspected_deployment_id": null}, "pipeline": {"reason": "No pipeline_run_id available from deployment evidence", "status": "NO_DATA"}, "deployment": {"reason": "Missing incident failure time or service for deployment correlation", "status": "NO_DATA", "correlation_method": "NO_DATA"}, "kubernetes": {"status": "COLLECTED", "cpu_status": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_status": null, "restart_delta": null, "crash_loop_count": null, "desired_replicas": null, "oom_killed_count": null, "pending_pod_count": null, "pod_restart_count": null, "available_replicas": null, "unavailable_replicas": null, "recent_warning_events": [], "failed_liveness_probe_count": null, "failed_readiness_probe_count": null}, "derived_facts": [{"fact_type": "SLO_BREACH", "description": "A reliability alert indicates the measured value breached the configured threshold.", "evidence_paths": ["slo.measured_value", "slo.target", "slo.alert_severity"]}], "evidence_hash": "32872f3d6c5cf6c4842cb64cf5ccdb6734c732194e5bc8624826a068467e4c07", "schema_version": "rca-evidence-bundle/v1", "missing_sources": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "collector_errors": [], "correlation_facts": [{"polarity": "CONTRADICTORY", "fact_type": "NO_POD_HEALTH_DEGRADATION", "description": "No pod restarts or readiness failures occurred.", "evidence_paths": ["kubernetes.pod_restart_count", "kubernetes.failed_readiness_probe_count"]}], "completeness_score": 0.75}	{"slo": "COLLECTED", "logs": "COLLECTED", "traces": "COLLECTED", "metrics": "PARTIAL", "pipeline": "NO_DATA", "deployment": "NO_DATA", "kubernetes": "COLLECTED"}	[]	\N	2026-08-02 09:49:16.109909+00	2026-08-02 09:56:17.874604+00
7ed212ec-8fd1-4fcf-a8fe-4b3505e2c628	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	14	PARTIAL	v1	\N	\N	\N	{"slo": {"status": "COLLECTED", "target": null, "slo_type": null, "burn_rate": null, "created_at": "2026-07-11T21:10:18.324058+00:00", "breach_status": null, "alert_severity": "HIGH", "measured_value": null, "window_minutes": null, "breach_started_at": null, "slo_definition_id": "4b572d35-cb52-47f2-9b91-f98577ade93a", "error_budget_status": null, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}, "logs": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_error_logs": 0, "error_count_after": 0, "error_count_before": 0, "out_of_memory_errors": 0, "top_error_signatures": [], "first_error_timestamp": null, "database_timeout_errors": 0, "connection_refused_errors": 0, "uncaught_exception_errors": 0}, "status": "PARTIAL", "traces": {"status": "COLLECTED", "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "total_traces": 0, "top_failed_span": null, "slow_trace_count": 0, "failed_trace_count": 0, "top_failed_dependency": null, "dependency_error_counts": {}, "failed_trace_percentage": 0, "p95_trace_duration_after": null, "representative_trace_ids": [], "p95_trace_duration_before": null, "p95_trace_duration_change": null}, "metrics": {"status": "PARTIAL", "queries": {"cpu": "container_cpu_usage", "memory": "container_memory_usage", "error_rate": "service_error_rate", "request_rate": "http_request_rate", "http_500_rate": "http_500_rate", "p95_latency_ms": "service_p95_latency_ms", "available_replicas": "kube_deployment_available_replicas"}, "service": null, "cpu_after": null, "cpu_before": null, "memory_after": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_before": null, "missing_series": ["request_rate", "http_500_rate", "error_rate", "p95_latency_ms", "cpu", "memory", "available_replicas"], "error_rate_after": null, "error_rate_before": null, "error_rate_change": null, "request_rate_after": null, "http_500_rate_after": null, "request_rate_before": null, "http_500_rate_before": null, "http_500_rate_change": null, "p95_latency_after_ms": null, "p95_latency_before_ms": null, "p95_latency_change_ms": null, "p95_latency_multiplier": null, "available_replicas_after": null, "available_replicas_before": null}, "incident": {"title": "demo-service error budget exhausted in staging", "status": "COLLECTED", "metrics": [], "severity": "SEV-2", "detected_at": "2026-07-11T21:29:10.350774+00:00", "environment": "staging", "incident_id": "c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b", "resolved_at": null, "acknowledged_at": "2026-08-02T09:48:25.901815+00:00", "incident_number": "INC-008", "incident_status": "REMEDIATING", "timeline_events": [{"source": "SYSTEM", "summary": null, "event_type": "INCIDENT_CREATED", "occurred_at": "2026-07-11T21:29:10.350774+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-07-19T20:08:24.031253+00:00"}, {"source": "USER", "summary": null, "event_type": "INCIDENT_COMMENT_ADDED", "occurred_at": "2026-07-19T20:14:35.886668+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACKNOWLEDGED", "occurred_at": "2026-08-02T09:48:25.901815+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_INVESTIGATING", "occurred_at": "2026-08-02T09:48:36.524654+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_ACTION_RECOMMENDED", "occurred_at": "2026-08-02T09:48:50.449715+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "INCIDENT_REMEDIATING", "occurred_at": "2026-08-02T09:48:55.019151+00:00"}, {"source": "OPERATOR", "summary": null, "event_type": "OPERATOR_ASSIGNED", "occurred_at": "2026-08-02T09:48:58.427571+00:00"}], "related_alert_ids": [], "failure_started_at": null, "primary_service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "human_context_included": false, "suspected_deployment_id": null}, "pipeline": {"reason": "No pipeline_run_id available from deployment evidence", "status": "NO_DATA"}, "deployment": {"reason": "Missing incident failure time or service for deployment correlation", "status": "NO_DATA", "correlation_method": "NO_DATA"}, "kubernetes": {"status": "COLLECTED", "cpu_status": null, "query_window": {"anchor": "2026-07-11T21:29:10.350774+00:00", "status": "COLLECTED", "after_end": "2026-07-11T21:44:10.350774+00:00", "before_end": "2026-07-11T21:29:10.350774+00:00", "after_start": "2026-07-11T21:29:10.350774+00:00", "before_start": "2026-07-11T21:14:10.350774+00:00"}, "memory_status": null, "restart_delta": null, "crash_loop_count": null, "desired_replicas": null, "oom_killed_count": null, "pending_pod_count": null, "pod_restart_count": null, "available_replicas": null, "unavailable_replicas": null, "recent_warning_events": [], "failed_liveness_probe_count": null, "failed_readiness_probe_count": null}, "derived_facts": [{"fact_type": "SLO_BREACH", "description": "A reliability alert indicates the measured value breached the configured threshold.", "evidence_paths": ["slo.measured_value", "slo.target", "slo.alert_severity"]}], "evidence_hash": "32872f3d6c5cf6c4842cb64cf5ccdb6734c732194e5bc8624826a068467e4c07", "schema_version": "rca-evidence-bundle/v1", "missing_sources": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "collector_errors": [], "correlation_facts": [{"polarity": "CONTRADICTORY", "fact_type": "NO_POD_HEALTH_DEGRADATION", "description": "No pod restarts or readiness failures occurred.", "evidence_paths": ["kubernetes.pod_restart_count", "kubernetes.failed_readiness_probe_count"]}], "completeness_score": 0.75}	{"slo": "COLLECTED", "logs": "COLLECTED", "traces": "COLLECTED", "metrics": "PARTIAL", "pipeline": "NO_DATA", "deployment": "NO_DATA", "kubernetes": "COLLECTED"}	[]	\N	2026-08-02 10:02:11.043515+00	2026-08-02 10:02:11.482253+00
\.
...
799d766e-71e9-4325-a078-0c677e9d24db	da71ef13-66cc-40fd-a81e-f30aa46388d6	RELIABILITY_ALERT	triggered_value	99.5	\N	RELIABILITY_ALERT	2026-07-16 19:31:20.479881+00	{"alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "alert_type": "ERROR_BUDGET_EXHAUSTED"}	2026-07-16 19:31:20.479881+00
c67c65e0-2d1b-4e54-afcc-8ddbfda5e1c3	da71ef13-66cc-40fd-a81e-f30aa46388d6	RELIABILITY_ALERT	threshold_value	99.9	\N	RELIABILITY_ALERT	2026-07-16 19:31:20.479881+00	{"alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "alert_type": "ERROR_BUDGET_EXHAUSTED"}	2026-07-16 19:31:20.479881+00
\.
...
COPY public.incident_timeline_events (id, incident_id, event_type, message, metadata_json, created_at, source, from_status, to_status, actor_user_id, alert_id, deployment_id, occurred_at) FROM stdin;
0e637306-3c2d-4a31-a87d-a998cdcbaf04	3fca917c-04e0-4db4-adfb-87e2588dcb09	INCIDENT_ACKNOWLEDGED	Incident acknowledged	{}	2026-07-04 21:12:29.934952+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-04 21:12:29.934952+00
d0c25e87-dfcc-4069-8722-8faa0d27bcee	3fca917c-04e0-4db4-adfb-87e2588dcb09	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "error_rate": 18.4, "snapshot_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "threshold_percent": 5}, "event_id": "evt_dcef17fb-1dd6-40c9-a9e2-e693a04978c6", "severity": "HIGH", "timestamp": "2026-07-04T21:06:16.892573+00:00", "event_type": "HIGH_ERROR_RATE", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "schema_version": "1.0"}	2026-07-04 21:06:16.899066+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-04 21:06:16.899066+00
187d0076-5ada-488f-953a-efe77d28afa3	3fca917c-04e0-4db4-adfb-87e2588dcb09	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "latency_ms": 2300.0, "snapshot_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "threshold_ms": 1000}, "event_id": "evt_bad15fe8-a1cc-443d-80a6-ae98be2f83a5", "severity": "HIGH", "timestamp": "2026-07-04T21:06:16.911209+00:00", "event_type": "HIGH_LATENCY", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "schema_version": "1.0"}	2026-07-04 21:06:16.911818+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-04 21:06:16.911818+00
5511eb1b-705a-4ba6-ac8d-dd3acc7f8b4f	3fca917c-04e0-4db4-adfb-87e2588dcb09	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "threshold": 3, "snapshot_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "pod_restart_count": 6}, "event_id": "evt_bb5a0876-b512-44b7-980c-4b4fc9b1baab", "severity": "HIGH", "timestamp": "2026-07-04T21:06:16.916696+00:00", "event_type": "POD_RESTART_SPIKE", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "schema_version": "1.0"}	2026-07-04 21:06:16.917355+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-04 21:06:16.917355+00
4dcffe79-ea74-46d4-b4b1-bea6d131864f	46ec3341-d948-4749-8bae-1e7f989d2f50	INCIDENT_ACKNOWLEDGED	Incident acknowledged	{}	2026-07-05 18:23:14.309506+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:23:14.309506+00
419c46a1-276d-46a0-922e-d5e34ad33051	46ec3341-d948-4749-8bae-1e7f989d2f50	INCIDENT_RESOLVED	Incident resolved	{}	2026-07-05 18:23:16.271965+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:23:16.271965+00
0e0f55f1-1799-4746-a518-ce8b7fcbf2c2	3fca917c-04e0-4db4-adfb-87e2588dcb09	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "severity": "MEDIUM", "snapshot_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "replica_count": 3, "available_replicas": 1}, "event_id": "evt_fc9763db-c133-4d21-bc3f-4770c1239e56", "severity": "MEDIUM", "timestamp": "2026-07-04T21:06:16.919517+00:00", "event_type": "SERVICE_DEGRADED", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "21aaf1ee-dff3-4463-8fd1-65cbf1853f51", "schema_version": "1.0"}	2026-07-04 21:06:16.920028+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-04 21:06:16.920028+00
65f8b530-5beb-408f-9fd8-8ca26b2f1c23	3fca917c-04e0-4db4-adfb-87e2588dcb09	INCIDENT_RESOLVED	Incident resolved	{}	2026-07-04 21:12:36.234452+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-04 21:12:36.234452+00
4dc85f9a-85f9-4e52-8cab-9ae5de7f7f35	a7db2d9c-c7a3-42df-90ea-d848ba238f7b	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "latency_ms": 2300.0, "snapshot_id": "79a37362-9300-45a1-b2ef-d7ec3794764e", "threshold_ms": 1000}, "event_id": "evt_7fd7dd56-e98d-4444-9826-98fa63749f99", "severity": "HIGH", "timestamp": "2026-07-05T16:37:37.404946+00:00", "event_type": "HIGH_LATENCY", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "79a37362-9300-45a1-b2ef-d7ec3794764e", "schema_version": "1.0"}	2026-07-05 16:37:37.414205+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 16:37:37.414205+00
8c681b5b-80c4-4b25-bf5e-66f740d2d754	a7db2d9c-c7a3-42df-90ea-d848ba238f7b	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "status": "UNHEALTHY", "severity": "CRITICAL", "snapshot_id": "79a37362-9300-45a1-b2ef-d7ec3794764e"}, "event_id": "evt_92a960d9-feac-41de-84c8-7dd59f68031d", "severity": "CRITICAL", "timestamp": "2026-07-05T16:37:37.427059+00:00", "event_type": "SERVICE_DOWN", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "79a37362-9300-45a1-b2ef-d7ec3794764e", "schema_version": "1.0"}	2026-07-05 16:37:37.428717+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 16:37:37.428717+00
2dbd276f-2266-48e8-beca-a82bce5a5186	1c8430b7-3fba-4f49-8e84-acdac4da8e5d	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "error_rate": 18.4, "snapshot_id": "74e66a5f-6c49-40ea-b5b3-3ed9e0ec6eda", "threshold_percent": 5}, "event_id": "evt_fe913dba-05b3-4960-af9e-8add89c2b370", "severity": "HIGH", "timestamp": "2026-07-05T16:38:30.526414+00:00", "event_type": "HIGH_ERROR_RATE", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "74e66a5f-6c49-40ea-b5b3-3ed9e0ec6eda", "schema_version": "1.0"}	2026-07-05 16:38:30.527683+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 16:38:30.527683+00
2504bc00-247f-4066-94b0-0a9bcfea278c	1c8430b7-3fba-4f49-8e84-acdac4da8e5d	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "status": "UNHEALTHY", "severity": "CRITICAL", "snapshot_id": "74e66a5f-6c49-40ea-b5b3-3ed9e0ec6eda"}, "event_id": "evt_3ec08cd2-b6f1-49f0-b1fb-fde2d7f4cbc9", "severity": "CRITICAL", "timestamp": "2026-07-05T16:38:30.532061+00:00", "event_type": "SERVICE_DOWN", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "74e66a5f-6c49-40ea-b5b3-3ed9e0ec6eda", "schema_version": "1.0"}	2026-07-05 16:38:30.533072+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 16:38:30.533072+00
67f2b56a-700e-49d3-83f9-349733bfbaea	caef74be-7b67-4764-9b93-71a6aea5edeb	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "threshold": 3, "snapshot_id": "a0a4d25b-012f-40dd-801a-9e059380cac1", "pod_restart_count": 7}, "event_id": "evt_9f705e94-54f7-4eed-b990-301dcf61e477", "severity": "HIGH", "timestamp": "2026-07-05T16:40:43.333462+00:00", "event_type": "POD_RESTART_SPIKE", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "a0a4d25b-012f-40dd-801a-9e059380cac1", "schema_version": "1.0"}	2026-07-05 16:40:43.335865+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 16:40:43.335865+00
7a825db2-5b18-4b63-a83d-a81f27cdf558	caef74be-7b67-4764-9b93-71a6aea5edeb	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "severity": "MEDIUM", "snapshot_id": "a0a4d25b-012f-40dd-801a-9e059380cac1", "replica_count": 3, "available_replicas": 2}, "event_id": "evt_ae0d0a2b-4435-4e6f-89cb-39c92c91d411", "severity": "MEDIUM", "timestamp": "2026-07-05T16:40:43.340718+00:00", "event_type": "SERVICE_DEGRADED", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "a0a4d25b-012f-40dd-801a-9e059380cac1", "schema_version": "1.0"}	2026-07-05 16:40:43.342209+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 16:40:43.342209+00
046bd781-5630-489a-83a3-50e63d58938e	f39c9a20-6fe5-46f8-af80-220082025abf	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "latency_ms": 2300.0, "snapshot_id": "2ae251be-c681-4f94-82d7-ba1919f31981", "threshold_ms": 1000}, "event_id": "evt_eb37f920-f5b0-4bc1-aef2-3dfcc17cfeae", "severity": "HIGH", "timestamp": "2026-07-05T17:06:29.700837+00:00", "event_type": "HIGH_LATENCY", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "2ae251be-c681-4f94-82d7-ba1919f31981", "schema_version": "1.0"}	2026-07-05 17:06:29.709137+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 17:06:29.709137+00
2efea15c-0605-42a5-a1f3-f1f399375c04	f39c9a20-6fe5-46f8-af80-220082025abf	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "status": "UNHEALTHY", "severity": "CRITICAL", "snapshot_id": "2ae251be-c681-4f94-82d7-ba1919f31981"}, "event_id": "evt_76056b05-df98-4deb-ba70-d6be16fad31a", "severity": "CRITICAL", "timestamp": "2026-07-05T17:06:29.722990+00:00", "event_type": "SERVICE_DOWN", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "2ae251be-c681-4f94-82d7-ba1919f31981", "schema_version": "1.0"}	2026-07-05 17:06:29.724107+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 17:06:29.724107+00
040a9735-27f2-4670-951c-9650b00bf7cc	f0fff6a0-aa8e-4bd4-be53-4af20d913a8d	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "error_rate": 18.4, "snapshot_id": "a6522068-03c8-46cd-93a4-741b1db36b7f", "threshold_percent": 5}, "event_id": "evt_893dc7b6-a556-4997-950a-2072f849eea8", "severity": "HIGH", "timestamp": "2026-07-05T17:07:35.278439+00:00", "event_type": "HIGH_ERROR_RATE", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "a6522068-03c8-46cd-93a4-741b1db36b7f", "schema_version": "1.0"}	2026-07-05 17:07:35.279875+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 17:07:35.279875+00
29b6bccf-4e8d-4bc7-83e5-7a92dd66bfbc	f0fff6a0-aa8e-4bd4-be53-4af20d913a8d	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "status": "UNHEALTHY", "severity": "CRITICAL", "snapshot_id": "a6522068-03c8-46cd-93a4-741b1db36b7f"}, "event_id": "evt_f765ba6c-3acf-465f-aee6-28b45fe33ef0", "severity": "CRITICAL", "timestamp": "2026-07-05T17:07:35.286018+00:00", "event_type": "SERVICE_DOWN", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "a6522068-03c8-46cd-93a4-741b1db36b7f", "schema_version": "1.0"}	2026-07-05 17:07:35.287609+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 17:07:35.287609+00
ce6a2c44-e1d1-4583-9199-fa717056c522	46ec3341-d948-4749-8bae-1e7f989d2f50	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-observability", "severity": "HIGH", "threshold": 3, "snapshot_id": "ef569b8b-3352-4a87-9f07-87249758ff99", "pod_restart_count": 7}, "event_id": "evt_0b9473db-d411-40e5-b9be-3ff68c1607b5", "severity": "HIGH", "timestamp": "2026-07-05T17:17:10.191675+00:00", "event_type": "POD_RESTART_SPIKE", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "ef569b8b-3352-4a87-9f07-87249758ff99", "schema_version": "1.0"}	2026-07-05 17:17:10.197243+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 17:17:10.197243+00
4373703d-5c7c-4f7f-8ed6-9d8d41defb94	46ec3341-d948-4749-8bae-1e7f989d2f50	INCIDENT_ALERT_ATTACHED	Additional telemetry alert attached to existing incident.	{"payload": {"source": "platformiq-observability", "severity": "MEDIUM", "snapshot_id": "ef569b8b-3352-4a87-9f07-87249758ff99", "replica_count": 3, "available_replicas": 2}, "event_id": "evt_88dcf0cc-1934-4479-8f92-53f185e0df34", "severity": "MEDIUM", "timestamp": "2026-07-05T17:17:10.205822+00:00", "event_type": "SERVICE_DEGRADED", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "service_name": "payment-service", "correlation_id": "ef569b8b-3352-4a87-9f07-87249758ff99", "schema_version": "1.0"}	2026-07-05 17:17:10.206818+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 17:17:10.206818+00
880cb3f9-621f-4a41-95b5-b672d7eb32c0	caef74be-7b67-4764-9b93-71a6aea5edeb	INCIDENT_ACKNOWLEDGED	Incident acknowledged	{}	2026-07-05 18:23:36.729816+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:23:36.729816+00
8da86f27-ff6a-4789-ada7-53ef4d8b2e63	caef74be-7b67-4764-9b93-71a6aea5edeb	INCIDENT_RESOLVED	Incident resolved	{}	2026-07-05 18:23:37.48733+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:23:37.48733+00
39e31ea5-696b-4b1f-9013-8fed1726ca99	f39c9a20-6fe5-46f8-af80-220082025abf	INCIDENT_RESOLVED	Incident resolved	{}	2026-07-05 18:24:13.221774+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:24:13.221774+00
159c6bca-9523-4b37-ab1e-2b9d17fbaad1	1c8430b7-3fba-4f49-8e84-acdac4da8e5d	INCIDENT_ACKNOWLEDGED	Incident acknowledged	{}	2026-07-05 18:24:55.581408+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:24:55.581408+00
5b1ad559-c225-444a-82a8-b72592b6dae0	1c8430b7-3fba-4f49-8e84-acdac4da8e5d	INCIDENT_RESOLVED	Incident resolved	{}	2026-07-05 18:25:02.168825+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-05 18:25:02.168825+00
add50a18-179d-419d-b225-e6c09244e631	a7db2d9c-c7a3-42df-90ea-d848ba238f7b	INCIDENT_ACKNOWLEDGED	Incident acknowledged	{}	2026-07-11 15:18:35.072358+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-11 15:18:35.072358+00
61fc12cc-1584-4105-afff-869c730ecbc6	a7db2d9c-c7a3-42df-90ea-d848ba238f7b	INCIDENT_RESOLVED	Incident resolved	{}	2026-07-11 15:18:36.902197+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-11 15:18:36.902197+00
65654359-c46b-4c86-96eb-df7b830c5f43	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	INCIDENT_CREATED	Incident created from telemetry alert.	{"payload": {"source": "platformiq-reliability", "severity": "HIGH", "burn_rate": 5.000000000000284, "rapid_burn": true, "is_breached": true, "metric_type": "AVAILABILITY", "service_name": "demo-service", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "measurement_id": "01917c13-827f-4224-a5a1-cb30fc86fcdc", "window_minutes": 60, "threshold_value": 99.9, "triggered_value": 99.5, "slo_definition_id": "4b572d35-cb52-47f2-9b91-f98577ade93a", "error_budget_status": "EXHAUSTED", "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "error_budget_consumed": 500.0000000000284, "error_budget_remaining": 0.0}, "event_id": "evt_f5ec3291-c1c5-426f-b2e1-b60de99d7074", "timestamp": "2026-07-11T21:10:18.381041+00:00", "event_type": "ERROR_BUDGET_EXHAUSTED", "service_id": "79a28492-b0d4-4098-94ce-70c4df2785cd", "environment": "staging", "correlation_id": "79a28492-b0d4-4098-94ce-70c4df2785cd:staging:AVAILABILITY", "schema_version": "1.0"}	2026-07-11 21:29:10.350774+00	SYSTEM	\N	\N	\N	\N	\N	2026-07-11 21:29:10.350774+00
2e5a44fd-a212-4eb4-8f04-0be532bb3c9a	da71ef13-66cc-40fd-a81e-f30aa46388d6	INCIDENT_DETECTED	Incident automatically detected from reliability alert d0391e45-448f-45b0-b978-be94302328b7.	{"alert_type": "ERROR_BUDGET_EXHAUSTED", "alert_severity": "HIGH", "deduplication_key": "79a28492-b0d4-4098-94ce-70c4df2785cd:staging:error_budget_exhausted", "incident_severity": "SEV-2"}	2026-07-16 19:31:20.479881+00	RELIABILITY_ALERT	\N	DETECTED	\N	d0391e45-448f-45b0-b978-be94302328b7	2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727	2026-07-11 21:10:18.324058+00
b62063fa-c693-4dc1-92ce-bd595f72aae2	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	OPERATOR_ASSIGNED	done	{"assigned_to_user_id": "8710aab9-161d-41e4-8149-d8c00772d426", "assigned_to_user_email": "hello@123.admin.com", "previous_assignee_user_id": null}	2026-07-19 20:08:24.025303+00	OPERATOR	\N	\N	8710aab9-161d-41e4-8149-d8c00772d426	\N	\N	2026-07-19 20:08:24.031253+00
99441718-f0ce-4309-935e-d6fcf8926ccf	f0fff6a0-aa8e-4bd4-be53-4af20d913a8d	OPERATOR_ASSIGNED	Hello checking it 123	{"assigned_to_user_id": "8710aab9-161d-41e4-8149-d8c00772d426", "assigned_to_user_email": "hello@123.admin.com", "previous_assignee_user_id": null}	2026-07-19 20:09:12.121722+00	OPERATOR	\N	\N	8710aab9-161d-41e4-8149-d8c00772d426	\N	\N	2026-07-19 20:09:12.123016+00
ee734d2b-a12c-4490-9b34-6d117422a250	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	INCIDENT_COMMENT_ADDED	hello@123.admin.com commented: Lets check	{"comment_id": "814af5e1-da59-4d78-ad03-fef289dbb5ca", "author_user_id": "8710aab9-161d-41e4-8149-d8c00772d426"}	2026-07-19 20:14:35.885018+00	USER	\N	\N	8710aab9-161d-41e4-8149-d8c00772d426	\N	\N	2026-07-19 20:14:35.886668+00
1c1873ed-cc8f-4cac-907f-6d2de868ef23	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	INCIDENT_ACKNOWLEDGED	Acknowledging it	{"assign_to_self": true, "assigned_to_user_id": "ec2c769f-e1d2-42e4-a0af-0cab146e19bd"}	2026-08-02 09:48:25.901506+00	OPERATOR	DETECTED	ACKNOWLEDGED	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	\N	\N	2026-08-02 09:48:25.901815+00
001cca88-1811-4777-b893-a7fa518e3824	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	INCIDENT_INVESTIGATING	Chanign it	{}	2026-08-02 09:48:36.524018+00	OPERATOR	ACKNOWLEDGED	INVESTIGATING	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	\N	\N	2026-08-02 09:48:36.524654+00
b2b003ec-8a4d-4d33-bfad-a0a9af38bcc2	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	INCIDENT_ACTION_RECOMMENDED	abc	{}	2026-08-02 09:48:50.449338+00	OPERATOR	INVESTIGATING	ACTION_RECOMMENDED	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	\N	\N	2026-08-02 09:48:50.449715+00
ebdf16cf-f276-4c39-a603-46cf33e99320	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	INCIDENT_REMEDIATING	d	{}	2026-08-02 09:48:55.018715+00	OPERATOR	ACTION_RECOMMENDED	REMEDIATING	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	\N	\N	2026-08-02 09:48:55.019151+00
7ad16f75-0560-4c92-8a6f-fe349c99140d	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	OPERATOR_ASSIGNED	Incident assigned to hello@123.admin.com.	{"assigned_to_user_id": "8710aab9-161d-41e4-8149-d8c00772d426", "assigned_to_user_email": "hello@123.admin.com", "previous_assignee_user_id": "ec2c769f-e1d2-42e4-a0af-0cab146e19bd"}	2026-08-02 09:48:58.425836+00	OPERATOR	\N	\N	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	\N	\N	2026-08-02 09:48:58.427571+00
\.
...
COPY public.incidents (id, title, description, severity, status, service_id, environment, correlation_id, triggered_by_event_id, started_at, resolved_at, created_at, updated_at, incident_number, primary_service_id, triggering_alert_id, suspected_deployment_id, deduplication_key, failure_started_at, detected_at, acknowledged_at, investigation_started_at, remediation_started_at, current_assignee_id, resolution_summary, rca_summary, remediation_summary, created_by) FROM stdin;
3fca917c-04e0-4db4-adfb-87e2588dcb09	payment-service high error rate in staging	Incident created from telemetry alert HIGH_ERROR_RATE for payment-service in staging.	SEV-2	RESOLVED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	21aaf1ee-dff3-4463-8fd1-65cbf1853f51	evt_dcef17fb-1dd6-40c9-a9e2-e693a04978c6	2026-07-04 21:06:16.899066	2026-07-04 21:12:36.233524+00	2026-07-04 21:06:16.899066+00	2026-07-04 21:12:36.233524+00	INC-001	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	21aaf1ee-dff3-4463-8fd1-65cbf1853f51	\N	2026-07-04 21:06:16.899066+00	\N	\N	\N	\N	\N	\N	\N	\N
a7db2d9c-c7a3-42df-90ea-d848ba238f7b	payment-service high latency in staging	Incident created from telemetry alert HIGH_LATENCY for payment-service in staging.	SEV-2	RESOLVED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	79a37362-9300-45a1-b2ef-d7ec3794764e	evt_7fd7dd56-e98d-4444-9826-98fa63749f99	2026-07-05 16:37:37.414205	2026-07-11 15:18:36.901495+00	2026-07-05 16:37:37.414205+00	2026-07-11 15:18:36.901495+00	INC-002	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	79a37362-9300-45a1-b2ef-d7ec3794764e	\N	2026-07-05 16:37:37.414205+00	\N	\N	\N	\N	\N	\N	\N	\N
1c8430b7-3fba-4f49-8e84-acdac4da8e5d	payment-service high error rate in staging	Incident created from telemetry alert HIGH_ERROR_RATE for payment-service in staging.	SEV-2	RESOLVED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	74e66a5f-6c49-40ea-b5b3-3ed9e0ec6eda	evt_fe913dba-05b3-4960-af9e-8add89c2b370	2026-07-05 16:38:30.527683	2026-07-05 18:25:02.168325+00	2026-07-05 16:38:30.527683+00	2026-07-05 18:25:02.168325+00	INC-003	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	74e66a5f-6c49-40ea-b5b3-3ed9e0ec6eda	\N	2026-07-05 16:38:30.527683+00	\N	\N	\N	\N	\N	\N	\N	\N
caef74be-7b67-4764-9b93-71a6aea5edeb	payment-service pod restart spike in staging	Incident created from telemetry alert POD_RESTART_SPIKE for payment-service in staging.	SEV-2	RESOLVED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	a0a4d25b-012f-40dd-801a-9e059380cac1	evt_9f705e94-54f7-4eed-b990-301dcf61e477	2026-07-05 16:40:43.335865	2026-07-05 18:23:37.485538+00	2026-07-05 16:40:43.335865+00	2026-07-05 18:23:37.485538+00	INC-004	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	a0a4d25b-012f-40dd-801a-9e059380cac1	\N	2026-07-05 16:40:43.335865+00	\N	\N	\N	\N	\N	\N	\N	\N
f39c9a20-6fe5-46f8-af80-220082025abf	payment-service high latency in staging	Incident created from telemetry alert HIGH_LATENCY for payment-service in staging.	SEV-2	RESOLVED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	2ae251be-c681-4f94-82d7-ba1919f31981	evt_eb37f920-f5b0-4bc1-aef2-3dfcc17cfeae	2026-07-05 17:06:29.709137	2026-07-05 18:24:13.21474+00	2026-07-05 17:06:29.709137+00	2026-07-05 18:24:13.21474+00	INC-005	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	2ae251be-c681-4f94-82d7-ba1919f31981	\N	2026-07-05 17:06:29.709137+00	\N	\N	\N	\N	\N	\N	\N	\N
46ec3341-d948-4749-8bae-1e7f989d2f50	payment-service pod restart spike in staging	Incident created from telemetry alert POD_RESTART_SPIKE for payment-service in staging.	SEV-2	RESOLVED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	ef569b8b-3352-4a87-9f07-87249758ff99	evt_0b9473db-d411-40e5-b9be-3ff68c1607b5	2026-07-05 17:17:10.197243	2026-07-05 18:23:16.269775+00	2026-07-05 17:17:10.197243+00	2026-07-05 18:23:16.269775+00	INC-007	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	ef569b8b-3352-4a87-9f07-87249758ff99	\N	2026-07-05 17:17:10.197243+00	\N	\N	\N	\N	\N	\N	\N	\N
f0fff6a0-aa8e-4bd4-be53-4af20d913a8d	payment-service high error rate in staging	Incident created from telemetry alert HIGH_ERROR_RATE for payment-service in staging.	SEV-2	DETECTED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	a6522068-03c8-46cd-93a4-741b1db36b7f	evt_893dc7b6-a556-4997-950a-2072f849eea8	2026-07-05 17:07:35.279875	\N	2026-07-05 17:07:35.279875+00	2026-07-19 20:09:12.121722+00	INC-006	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	a6522068-03c8-46cd-93a4-741b1db36b7f	\N	2026-07-05 17:07:35.279875+00	\N	\N	\N	8710aab9-161d-41e4-8149-d8c00772d426	\N	\N	\N	\N
da71ef13-66cc-40fd-a81e-f30aa46388d6	Error Budget Exhausted: demo-service	Automatically created from reliability alert d0391e45-448f-45b0-b978-be94302328b7.	SEV-2	DETECTED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:error_budget_exhausted	d0391e45-448f-45b0-b978-be94302328b7	\N	\N	2026-07-16 19:31:20.479881+00	2026-07-16 19:31:20.479881+00	INC-009	79a28492-b0d4-4098-94ce-70c4df2785cd	d0391e45-448f-45b0-b978-be94302328b7	2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:error_budget_exhausted	\N	2026-07-11 21:10:18.324058+00	\N	\N	\N	\N	\N	\N	\N	\N
c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	demo-service error budget exhausted in staging	Incident created from telemetry alert error budget exhausted for demo-service in staging.	SEV-2	REMEDIATING	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:AVAILABILITY	evt_f5ec3291-c1c5-426f-b2e1-b60de99d7074	2026-07-11 21:29:10.350774	\N	2026-07-11 21:29:10.350774+00	2026-08-02 09:48:58.425836+00	INC-008	79a28492-b0d4-4098-94ce-70c4df2785cd	\N	\N	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:AVAILABILITY	\N	2026-07-11 21:29:10.350774+00	2026-08-02 09:48:25.901815+00	2026-08-02 09:48:36.524654+00	2026-08-02 09:48:55.019151+00	8710aab9-161d-41e4-8149-d8c00772d426	\N	\N	\N	\N
4df47d4d-723b-48d7-a5d4-e8985cbe1544	Payment service reliability regression	Error rate increased after deployment.	SEV-3	RESOLVED	\N	staging	\N	\N	\N	2026-08-02 10:14:50.695161+00	2026-08-02 10:08:15.003589+00	2026-08-02 10:14:50.661305+00	INC-010	b350a986-e527-4825-9efb-99313d9abf09	\N	\N	\N	\N	2026-08-02 10:08:15.003589+00	\N	\N	\N	\N	Recovery verified after remediation.	\N	Service recovery verified after ROLLBACK_DEPLOYMENT.	ec2c769f-e1d2-42e4-a0af-0cab146e19bd
\.
...
e1b7e061-e7dc-47eb-a160-c90e88c72e7c	evt_8bb2a21c-f1ea-42fc-9fd3-550e22fd6869	telemetry.alerts	RELIABILITY_ALERT_CREATED	1.0	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:ERROR_BUDGET_EXHAUSTED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	{"source": "sprint-7d-e2e-test", "alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "severity": "HIGH", "alert_type": "ERROR_BUDGET_EXHAUSTED", "created_at": "2026-07-11T21:10:18.324058+00:00", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "threshold_value": 99.9, "triggered_value": 99.5, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}	PUBLISHED	0	\N	2026-07-16 19:31:20.352412+00	2026-07-16 19:31:15.654431+00	2026-07-16 19:31:20.352441+00
66060259-d5ac-4ceb-8281-776fe8063571	evt_fce541a1-eb72-416a-a42d-51b131ea7adf	telemetry.alerts	RELIABILITY_ALERT_CREATED	1.0	79a28492-b0d4-4098-94ce-70c4df2785cd:staging:ERROR_BUDGET_EXHAUSTED	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	{"source": "sprint-7d-duplicate-test", "alert_id": "d0391e45-448f-45b0-b978-be94302328b7", "severity": "HIGH", "alert_type": "ERROR_BUDGET_EXHAUSTED", "deployment_id": "2d1f11ac-dac7-4a9f-a8e0-e62a0bb77727", "threshold_value": 99.9, "triggered_value": 99.5, "reliability_alert_id": "d0391e45-448f-45b0-b978-be94302328b7"}	PUBLISHED	0	\N	2026-07-16 19:34:28.205591+00	2026-07-16 19:34:21.848092+00	2026-07-16 19:34:28.205609+00
bb85fd95-36d4-4eec-b08b-415b085be9f3	evt_d2f2c22b-8fc5-42b2-be0e-b64ab3d6f1e6	telemetry.alerts	RELIABILITY_ALERT_CREATED	1.0	sprint-7d-failure-test	79a28492-b0d4-4098-94ce-70c4df2785cd	staging	{"source": "sprint-7d-failure-test", "alert_id": "missing-alert-7d-test", "severity": "HIGH", "alert_type": "LATENCY_BREACH", "threshold_value": 500.0, "triggered_value": 2300.0, "reliability_alert_id": "missing-alert-7d-test"}	PUBLISHED	0	\N	2026-07-16 19:36:16.612114+00	2026-07-16 19:36:11.158417+00	2026-07-16 19:36:16.612131+00
d322a015-20b7-478a-8856-d94d2cb528bc	evt_27dec9e4-5df0-40f1-ad0d-9d1f40f7adec	remediation.events	REMEDIATION_APPROVED	1.0	4df47d4d-723b-48d7-a5d4-e8985cbe1544	b350a986-e527-4825-9efb-99313d9abf09	staging	{"status": "APPROVED", "decision": "APPROVED", "decided_by": "ec2c769f-e1d2-42e4-a0af-0cab146e19bd", "service_id": "b350a986-e527-4825-9efb-99313d9abf09", "action_type": "ROLLBACK_DEPLOYMENT", "approval_id": "3628440e-e946-4c08-816d-6c44514db7c5", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "occurred_at": "2026-08-02T10:11:39.240910+00:00", "approval_only": true, "event_version": 1, "rejection_reason": null, "recommendation_id": "ad63d41f-a383-4bcf-941f-c2c87e3e8dff", "execution_requested": false, "human_decision_recorded": true, "requires_human_approval": true}	PENDING	0	\N	\N	2026-08-02 10:11:39.548181+00	2026-08-02 10:11:39.548329+00
1e7857d7-ea13-4101-bd92-75ee0fe15c98	evt_f175e4a0-de73-458b-bae8-e674adbc088a	remediation.commands	ROLLBACK_STARTED	1.0	4df47d4d-723b-48d7-a5d4-e8985cbe1544	b350a986-e527-4825-9efb-99313d9abf09	staging	{"service_id": "b350a986-e527-4825-9efb-99313d9abf09", "action_type": "ROLLBACK_DEPLOYMENT", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "occurred_at": "2026-08-02T10:13:01.197620+00:00", "command_type": "ARGOCD_ROLLBACK", "execution_id": "d38dc37d-5c59-4d4c-91e9-9051f8a309c5", "event_version": 1, "command_payload": {"simulated": true, "service_id": "b350a986-e527-4825-9efb-99313d9abf09", "action_type": "ROLLBACK_DEPLOYMENT", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "command_type": "ARGOCD_ROLLBACK", "deployment_id": null, "target_revision": "v1.8.1", "recommendation_id": "ad63d41f-a383-4bcf-941f-c2c87e3e8dff"}, "execution_status": "IN_PROGRESS", "recommendation_id": "ad63d41f-a383-4bcf-941f-c2c87e3e8dff"}	PENDING	0	\N	\N	2026-08-02 10:13:01.207939+00	2026-08-02 10:13:01.207942+00
cd460dfb-abd9-435f-991b-a7b5337b7862	evt_d2bed500-246e-428c-b751-b81c679f451a	remediation.results	ROLLBACK_COMPLETED	1.0	4df47d4d-723b-48d7-a5d4-e8985cbe1544	b350a986-e527-4825-9efb-99313d9abf09	staging	{"result": {"status": "COMPLETED", "message": "Rollback command accepted.", "command_type": "ARGOCD_ROLLBACK", "target_revision": "v1.8.1"}, "service_id": "b350a986-e527-4825-9efb-99313d9abf09", "action_type": "ROLLBACK_DEPLOYMENT", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "occurred_at": "2026-08-02T10:13:01.207319+00:00", "command_type": "ARGOCD_ROLLBACK", "execution_id": "d38dc37d-5c59-4d4c-91e9-9051f8a309c5", "event_version": 1, "execution_status": "SUCCEEDED", "recommendation_id": "ad63d41f-a383-4bcf-941f-c2c87e3e8dff"}	PENDING	0	\N	\N	2026-08-02 10:13:01.211925+00	2026-08-02 10:13:01.211928+00
148ff04f-d445-4bfd-850f-9004c1238802	evt_75adfbb3-4cc7-495e-960b-0e8068a9a0c5	remediation.results	RECOVERY_VERIFIED	1.0	4df47d4d-723b-48d7-a5d4-e8985cbe1544	b350a986-e527-4825-9efb-99313d9abf09	staging	{"service_id": "b350a986-e527-4825-9efb-99313d9abf09", "action_type": "ROLLBACK_DEPLOYMENT", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "occurred_at": "2026-08-02T10:14:50.695161+00:00", "execution_id": "d38dc37d-5c59-4d4c-91e9-9051f8a309c5", "pods_healthy": true, "event_version": 1, "verification_id": "a0a4f6c0-2c91-436b-a016-4ad6d14920b3", "metrics_snapshot": {"status": "HEALTHY", "error_rate": 0.2, "thresholds": {"maximum_error_rate": 1.0, "maximum_p95_latency_ms": 500.0}, "observed_at": "2026-08-02T10:14:45.159808+00:00", "replica_count": 3, "p95_latency_ms": 180.0, "pod_restart_count": 0, "available_replicas": 3, "health_snapshot_id": "6ffa0181-b8c5-4a14-b091-cb5ff3211bf4", "previous_pod_restart_count": null}, "latency_recovered": true, "recommendation_id": "ad63d41f-a383-4bcf-941f-c2c87e3e8dff", "restart_loop_absent": true, "verification_status": "VERIFIED", "error_rate_recovered": true, "availability_restored": true}	PENDING	0	\N	\N	2026-08-02 10:14:50.754696+00	2026-08-02 10:14:50.754759+00
\.
...
34b91385-98f7-4bda-ab52-445c9a5219c6	Sprint 9C Project 231417d2-6848-4001-b4f0-dada6b90752f	Project used by remediation approval tests.	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	2026-08-02 10:08:15.005607
\.


--
-- Data for Name: rca_feedback; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.rca_feedback (id, incident_id, report_id, rating, comment, correct_root_cause, reviewer_id, created_at, updated_at) FROM stdin;
\.
...
COPY public.rca_reports (id, incident_id, evidence_id, version, status, schema_version, confidence, summary, probable_root_cause, supporting_evidence, contradictory_evidence, alternative_hypotheses, missing_evidence, model_provider, model_name, prompt_version, generated_at, created_at, updated_at, report_json) FROM stdin;
34ac887a-fffa-4c78-b01f-a869e5ad4a82	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	b6716bb2-78e2-468b-a65b-0e31526377be	1	PENDING	1.0	\N	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 10:41:33.164082+00	2026-07-26 10:41:33.164082+00	{}
8cf6c4f8-f398-4270-ac44-2f33f59d824f	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	1bf76375-6f58-429f-b750-70b5abb985b2	2	FAILED	1.0	\N	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 10:50:52.837621+00	2026-07-26 10:50:53.056791+00	{}
8fbb3d5e-2c5b-4c6d-9ba9-415309a5a160	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	fdbb7887-7cbf-48a6-a586-4c0d53b995a7	3	FAILED	1.0	\N	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 10:57:50.214315+00	2026-07-26 10:57:50.874495+00	{}
e736ef2e-55ba-4340-a4fe-d76a6e4fd176	da71ef13-66cc-40fd-a81e-f30aa46388d6	c464bf88-363e-43da-afea-01b51f0e4d21	1	FAILED	1.0	\N	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 10:58:36.748593+00	2026-07-26 10:58:36.777979+00	{}
a5cd8aa7-ee8f-4ed9-900f-44494101d7b5	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	7a94d9b1-0fea-4798-b30f-75b49717bbc3	4	FAILED	1.0	\N	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 11:01:56.138825+00	2026-07-26 11:01:56.320868+00	{}
d447759b-d9b4-4052-85ec-315c554bcad8	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	86df0962-45f3-46a6-a91b-cb7b862161b7	5	COMPLETED	1.0	LOW	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 11:04:42.605742+00	2026-07-26 11:04:42.791775+00	{}
4988dbb0-0320-43c3-8195-6c9c83a1ce30	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	7ed212ec-8fd1-4fcf-a8fe-4b3505e2c628	14	COMPLETED	1.0	LOW	Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.	RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.	[{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}]	[]	[]	[{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}]	\N	gpt-4.1-mini	rca_v1	2026-08-02 10:02:11.567599+00	2026-08-02 10:02:11.043515+00	2026-08-02 10:02:11.570328+00	{"confidence": "LOW", "failure_reason": "9 validation errors for IncidentEvidenceBundle\\ntime_window\\n  Field required [type=missing, input_value={'schema_version': 'rca-e...5bc8624826a068467e4c07'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ndeployment.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...tion_method': 'NO_DATA'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\npipeline.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...om deployment evidence'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nslo.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...T21:10:18.324058+00:00'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nmetrics.metadata\\n  Field required [type=missing, input_value={'status': 'PARTIAL', 'se..., 'available_replicas']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nlogs.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '... 'error_count_after': 0}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ntraces.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...entative_trace_ids': []}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nkubernetes.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '..., 'memory_status': None}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nderived_facts.0.fact\\n  Field required [type=missing, input_value={'fact_type': 'SLO_BREACH..., 'slo.alert_severity']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing", "missing_evidence": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "probable_root_cause": "RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.", "recommended_actions": ["Recommended investigation: align evidence collector output with IncidentEvidenceBundle schema.", "Suggested validation step: ensure time-window, source metadata, and derived-fact fields are present."], "root_cause_category": "INSUFFICIENT_STRUCTURED_EVIDENCE", "supporting_evidence": [{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}], "alternative_hypotheses": [], "confidence_explanation": "Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.", "contradictory_evidence": []}
0265681c-e95c-4700-92a8-512e83f4d100	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	950dcdd1-8f15-4040-9bfd-20e9c21e625d	6	COMPLETED	1.0	LOW	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 11:09:16.963854+00	2026-07-26 11:09:17.318756+00	{}
6bfb9534-c719-4676-8396-0e6c77f4fafb	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	856a0caf-613e-42ee-af77-be3213bf2938	7	COMPLETED	1.0	LOW	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 11:25:30.247565+00	2026-07-26 11:25:30.698677+00	{}
9b53220d-cde8-4123-9eba-f7e6e727af58	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	52a4820d-f857-44ce-8fa0-eac9581e0344	8	COMPLETED	1.0	LOW	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 11:32:44.486029+00	2026-07-26 11:32:44.863358+00	{}
b37491a5-d981-4341-806b-3b0d9508ac00	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	1eb1c41a-45f4-4f98-aae8-52a959acd1c0	9	COMPLETED	1.0	LOW	\N	\N	[]	[]	[]	[]	\N	\N	rca_v1	\N	2026-07-26 11:33:33.34458+00	2026-07-26 11:33:33.370873+00	{}
54a6918d-cd49-429a-920b-6923c726fc22	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	b184e6bb-91df-419d-b058-5b81420f91b0	10	COMPLETED	1.0	LOW	Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.	RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.	[{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}]	[]	[]	[{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}]	\N	gpt-4.1-mini	rca_v1	2026-08-02 09:56:17.674035+00	2026-08-02 09:47:49.607921+00	2026-08-02 09:56:17.6761+00	{"confidence": "LOW", "failure_reason": "9 validation errors for IncidentEvidenceBundle\\ntime_window\\n  Field required [type=missing, input_value={'schema_version': 'rca-e...5bc8624826a068467e4c07'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ndeployment.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...tion_method': 'NO_DATA'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\npipeline.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...om deployment evidence'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nslo.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...T21:10:18.324058+00:00'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nmetrics.metadata\\n  Field required [type=missing, input_value={'status': 'PARTIAL', 'se..., 'available_replicas']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nlogs.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '... 'error_count_after': 0}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ntraces.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...entative_trace_ids': []}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nkubernetes.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '..., 'memory_status': None}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nderived_facts.0.fact\\n  Field required [type=missing, input_value={'fact_type': 'SLO_BREACH..., 'slo.alert_severity']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing", "missing_evidence": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "probable_root_cause": "RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.", "recommended_actions": ["Recommended investigation: align evidence collector output with IncidentEvidenceBundle schema.", "Suggested validation step: ensure time-window, source metadata, and derived-fact fields are present."], "root_cause_category": "INSUFFICIENT_STRUCTURED_EVIDENCE", "supporting_evidence": [{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}], "alternative_hypotheses": [], "confidence_explanation": "Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.", "contradictory_evidence": []}
29388d3c-59bc-4d94-8951-ced5e3158ca1	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	d77cd1a6-683d-4eb7-b646-f1c659485844	12	COMPLETED	1.0	LOW	Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.	RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.	[{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}]	[]	[]	[{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}]	\N	gpt-4.1-mini	rca_v1	2026-08-02 09:56:17.673993+00	2026-08-02 09:49:25.60716+00	2026-08-02 09:56:17.676058+00	{"confidence": "LOW", "failure_reason": "9 validation errors for IncidentEvidenceBundle\\ntime_window\\n  Field required [type=missing, input_value={'schema_version': 'rca-e...5bc8624826a068467e4c07'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ndeployment.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...tion_method': 'NO_DATA'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\npipeline.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...om deployment evidence'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nslo.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...T21:10:18.324058+00:00'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nmetrics.metadata\\n  Field required [type=missing, input_value={'status': 'PARTIAL', 'se..., 'available_replicas']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nlogs.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '... 'error_count_after': 0}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ntraces.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...entative_trace_ids': []}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nkubernetes.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '..., 'memory_status': None}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nderived_facts.0.fact\\n  Field required [type=missing, input_value={'fact_type': 'SLO_BREACH..., 'slo.alert_severity']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing", "missing_evidence": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "probable_root_cause": "RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.", "recommended_actions": ["Recommended investigation: align evidence collector output with IncidentEvidenceBundle schema.", "Suggested validation step: ensure time-window, source metadata, and derived-fact fields are present."], "root_cause_category": "INSUFFICIENT_STRUCTURED_EVIDENCE", "supporting_evidence": [{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}], "alternative_hypotheses": [], "confidence_explanation": "Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.", "contradictory_evidence": []}
ce06bf6b-d5d5-4ad7-9d7a-a53b3374d7a2	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	81317a1c-ee99-469d-a3d1-f6681a678a6c	13	COMPLETED	1.0	LOW	Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.	RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.	[{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}]	[]	[]	[{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}]	\N	gpt-4.1-mini	rca_v1	2026-08-02 09:56:17.90089+00	2026-08-02 09:51:58.134619+00	2026-08-02 09:56:17.902071+00	{"confidence": "LOW", "failure_reason": "9 validation errors for IncidentEvidenceBundle\\ntime_window\\n  Field required [type=missing, input_value={'schema_version': 'rca-e...5bc8624826a068467e4c07'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ndeployment.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...tion_method': 'NO_DATA'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\npipeline.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...om deployment evidence'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nslo.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...T21:10:18.324058+00:00'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nmetrics.metadata\\n  Field required [type=missing, input_value={'status': 'PARTIAL', 'se..., 'available_replicas']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nlogs.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '... 'error_count_after': 0}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ntraces.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...entative_trace_ids': []}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nkubernetes.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '..., 'memory_status': None}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nderived_facts.0.fact\\n  Field required [type=missing, input_value={'fact_type': 'SLO_BREACH..., 'slo.alert_severity']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing", "missing_evidence": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "probable_root_cause": "RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.", "recommended_actions": ["Recommended investigation: align evidence collector output with IncidentEvidenceBundle schema.", "Suggested validation step: ensure time-window, source metadata, and derived-fact fields are present."], "root_cause_category": "INSUFFICIENT_STRUCTURED_EVIDENCE", "supporting_evidence": [{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}], "alternative_hypotheses": [], "confidence_explanation": "Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.", "contradictory_evidence": []}
a70f0122-bef5-4be9-b48e-abf78e64553e	c96510c1-4c91-4c0d-bbab-c8eb19dc5f8b	56645ecc-93cc-4a2b-801e-493fd15b28c3	11	COMPLETED	1.0	LOW	Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.	RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.	[{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}]	[]	[]	[{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}]	\N	gpt-4.1-mini	rca_v1	2026-08-02 09:56:17.908993+00	2026-08-02 09:49:16.109909+00	2026-08-02 09:56:17.910357+00	{"confidence": "LOW", "failure_reason": "9 validation errors for IncidentEvidenceBundle\\ntime_window\\n  Field required [type=missing, input_value={'schema_version': 'rca-e...5bc8624826a068467e4c07'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ndeployment.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...tion_method': 'NO_DATA'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\npipeline.metadata\\n  Field required [type=missing, input_value={'status': 'NO_DATA', 're...om deployment evidence'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nslo.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...T21:10:18.324058+00:00'}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nmetrics.metadata\\n  Field required [type=missing, input_value={'status': 'PARTIAL', 'se..., 'available_replicas']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nlogs.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '... 'error_count_after': 0}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\ntraces.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '...entative_trace_ids': []}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nkubernetes.metadata\\n  Field required [type=missing, input_value={'status': 'COLLECTED', '..., 'memory_status': None}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing\\nderived_facts.0.fact\\n  Field required [type=missing, input_value={'fact_type': 'SLO_BREACH..., 'slo.alert_severity']}, input_type=dict]\\n    For further information visit https://errors.pydantic.dev/2.13/v/missing", "missing_evidence": [{"reason": "Missing incident failure time or service for deployment correlation", "source": "deployment", "status": "NO_DATA"}, {"reason": "No pipeline_run_id available from deployment evidence", "source": "pipeline", "status": "NO_DATA"}], "probable_root_cause": "RCA generation could not produce a strict LLM report because the collected evidence bundle did not match the expected RCA schema.", "recommended_actions": ["Recommended investigation: align evidence collector output with IncidentEvidenceBundle schema.", "Suggested validation step: ensure time-window, source metadata, and derived-fact fields are present."], "root_cause_category": "INSUFFICIENT_STRUCTURED_EVIDENCE", "supporting_evidence": [{"summary": "Evidence collection completed but schema validation failed.", "evidence_path": "evidence_bundle"}], "alternative_hypotheses": [], "confidence_explanation": "Confidence is low because evidence was collected, but the bundle failed strict validation before LLM report generation.", "contradictory_evidence": []}
\.


--
-- Data for Name: recovery_verifications; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.recovery_verifications (id, remediation_id, remediation_execution_id, verification_status, error_rate_recovered, latency_recovered, pods_healthy, restart_loop_absent, availability_restored, metrics_snapshot, verified_at, created_at) FROM stdin;
a0a4f6c0-2c91-436b-a016-4ad6d14920b3	ad63d41f-a383-4bcf-941f-c2c87e3e8dff	d38dc37d-5c59-4d4c-91e9-9051f8a309c5	VERIFIED	t	t	t	t	t	{"status": "HEALTHY", "error_rate": 0.2, "thresholds": {"maximum_error_rate": 1.0, "maximum_p95_latency_ms": 500.0}, "observed_at": "2026-08-02T10:14:45.159808+00:00", "replica_count": 3, "p95_latency_ms": 180.0, "pod_restart_count": 0, "available_replicas": 3, "health_snapshot_id": "6ffa0181-b8c5-4a14-b091-cb5ff3211bf4", "previous_pod_restart_count": null}	2026-08-02 10:14:50.695161+00	2026-08-02 10:14:50.661305+00
\.
...
--
-- Data for Name: remediation_approvals; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.remediation_approvals (id, remediation_id, approved_by, decision, rejection_reason, approved_at) FROM stdin;
3628440e-e946-4c08-816d-6c44514db7c5	ad63d41f-a383-4bcf-941f-c2c87e3e8dff	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	APPROVED	\N	2026-08-02 10:11:39.24091+00
\.


--
-- Data for Name: remediation_executions; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.remediation_executions (id, remediation_id, command_type, command_payload, execution_status, started_at, completed_at, result_summary, error_message, created_at) FROM stdin;
d38dc37d-5c59-4d4c-91e9-9051f8a309c5	ad63d41f-a383-4bcf-941f-c2c87e3e8dff	ROLLBACK_DEPLOYMENT	{"simulated": true, "service_id": "b350a986-e527-4825-9efb-99313d9abf09", "action_type": "ROLLBACK_DEPLOYMENT", "environment": "staging", "incident_id": "4df47d4d-723b-48d7-a5d4-e8985cbe1544", "command_type": "ARGOCD_ROLLBACK", "deployment_id": null, "target_revision": "v1.8.1", "recommendation_id": "ad63d41f-a383-4bcf-941f-c2c87e3e8dff"}	SUCCEEDED	2026-08-02 10:13:01.19762+00	2026-08-02 10:13:01.207319+00	{"status": "COMPLETED", "message": "Rollback command accepted.", "command_type": "ARGOCD_ROLLBACK", "target_revision": "v1.8.1"}	\N	2026-08-02 10:13:01.178812+00
\.


--
-- Data for Name: remediation_recommendations; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.remediation_recommendations (id, incident_id, service_id, environment, action_type, reason, evidence_summary, confidence, status, created_by, created_at, updated_at) FROM stdin;
ad63d41f-a383-4bcf-941f-c2c87e3e8dff	4df47d4d-723b-48d7-a5d4-e8985cbe1544	b350a986-e527-4825-9efb-99313d9abf09	staging	ROLLBACK_DEPLOYMENT	A recent deployment is correlated with the reliability regression.	{"rule_code": "RECENT_DEPLOYMENT_REGRESSION", "previous_revision": "v1.8.1", "deployment_revision": "v1.8.2"}	HIGH	RECOVERY_VERIFIED	ec2c769f-e1d2-42e4-a0af-0cab146e19bd	2026-08-02 10:08:15.003589+00	2026-08-02 10:14:50.661305+00
\.
...
a2e47fa5-5d24-4fee-a598-f9d768d20010	admin	2026-07-19 19:00:06.036378
cdf9b4ad-2cd1-4be9-9df7-3746a40d98c3	developer	2026-07-19 19:00:06.036381
a59ed5cd-db79-4516-b2c6-a6c8c2100c18	operator	2026-07-19 19:00:06.036381
5e0b8190-a11a-4c64-978b-86737ce4ea35	viewer	2026-07-19 19:00:06.036381
\.
...
6ffa0181-b8c5-4a14-b091-cb5ff3211bf4	b350a986-e527-4825-9efb-99313d9abf09	payment-service-3a1841d0-0a7f-4e79-8d2a-ec9913fb7589	staging	HEALTHY	180	0.2	35	40	0	3	3	sprint9g-ui-demo	2026-08-02 10:14:45.159808+00
\.
...
b350a986-e527-4825-9efb-99313d9abf09	34b91385-98f7-4bda-ab52-445c9a5219c6	payment-service-3a1841d0-0a7f-4e79-8d2a-ec9913fb7589	Service used by remediation approval tests.	BACKEND	platform-team	2026-08-02 10:08:15.010261
\.
...
34bd2bdf-e2c0-4957-8169-9f1a73a635b2	5e0b8190-a11a-4c64-978b-86737ce4ea35
8710aab9-161d-41e4-8149-d8c00772d426	a2e47fa5-5d24-4fee-a598-f9d768d20010
97693566-4644-4bce-9273-fb21dd673d42	a2e47fa5-5d24-4fee-a598-f9d768d20010
cad7d05b-fba7-4c81-852e-23e50a1fadb9	a2e47fa5-5d24-4fee-a598-f9d768d20010
5d1094b1-951c-437e-ac42-f1f0b57911be	a2e47fa5-5d24-4fee-a598-f9d768d20010
ec2c769f-e1d2-42e4-a0af-0cab146e19bd	a2e47fa5-5d24-4fee-a598-f9d768d20010
\.
...
34bd2bdf-e2c0-4957-8169-9f1a73a635b2	viewer@123.com	$2b$12$WsZm/Dzgj6tuO9NA5jaBW.wV/jZpnvNKjkHgMfE5qpgYhkScDfrwO	\N	t	2026-07-19 19:02:19.854692
8710aab9-161d-41e4-8149-d8c00772d426	hello@123.admin.com	$2b$12$RJskOnyd/7K/rcVdK/WsJOcXo606KAgVv.hnfUwVHgOIfZ2qpYkby	\N	t	2026-07-19 19:49:10.826354
97693566-4644-4bce-9273-fb21dd673d42	hello2@admin123.com	$2b$12$kEhxXsbtm03yu5v2UUqsTekCRqKbIHvRbHINGVu0FOxy2NbKj.Aqi	\N	t	2026-07-19 20:06:17.952301
cad7d05b-fba7-4c81-852e-23e50a1fadb9	shounak.chandra@admin.com	$2b$12$vYkVnIcEFMZoUt1xeT7CbeoN1KdFnAUdv/lHR6sCsnWUixQVKBGiS	\N	t	2026-07-26 10:31:55.058169
5d1094b1-951c-437e-ac42-f1f0b57911be	admin@123.com	$2b$12$faVAPPG2uFNUJyETLforReHlGbZMkp7dpw25VqSnJCedoVL.fTyG6	\N	t	2026-07-26 10:32:41.043701
ec2c769f-e1d2-42e4-a0af-0cab146e19bd	shounak@admin.com	$2b$12$DGadCJPykFClpDDqzFd1fuugyM5tvlHXRS.DVeWQ0kfdwbq21O8kK	\N	t	2026-08-02 09:45:10.32787
\.
...
SELECT pg_catalog.setval('public.incident_number_seq', 10, true);
...
--
-- Name: chaos_observations chaos_observations_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_observations
    ADD CONSTRAINT chaos_observations_pkey PRIMARY KEY (id);


--
-- Name: chaos_runs chaos_runs_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_pkey PRIMARY KEY (id);
...
--
-- Name: experiment_benchmarks experiment_benchmarks_chaos_run_id_key; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.experiment_benchmarks
    ADD CONSTRAINT experiment_benchmarks_chaos_run_id_key UNIQUE (chaos_run_id);


--
-- Name: experiment_benchmarks experiment_benchmarks_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.experiment_benchmarks
    ADD CONSTRAINT experiment_benchmarks_pkey PRIMARY KEY (id);


--
-- Name: incident_timeline_events incident_events_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: incident_alert_links pk_incident_alert_links; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_alert_links
    ADD CONSTRAINT pk_incident_alert_links PRIMARY KEY (id);


--
-- Name: incident_assignments pk_incident_assignments; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_assignments
    ADD CONSTRAINT pk_incident_assignments PRIMARY KEY (id);


--
-- Name: incident_comments pk_incident_comments; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_comments
    ADD CONSTRAINT pk_incident_comments PRIMARY KEY (id);


--
-- Name: incident_metrics pk_incident_metrics; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_metrics
    ADD CONSTRAINT pk_incident_metrics PRIMARY KEY (id);
...
--
-- Name: rca_feedback rca_feedback_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_feedback
...
--
-- Name: recovery_verifications recovery_verifications_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.recovery_verifications
    ADD CONSTRAINT recovery_verifications_pkey PRIMARY KEY (id);
...
--
-- Name: remediation_approvals remediation_approvals_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: remediation_executions remediation_executions_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: remediation_recommendations remediation_recommendations_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_recommendations
    ADD CONSTRAINT remediation_recommendations_pkey PRIMARY KEY (id);
...
--
-- Name: incident_alert_links uq_incident_alert_links_reliability_alert_id; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_alert_links
    ADD CONSTRAINT uq_incident_alert_links_reliability_alert_id UNIQUE (reliability_alert_id);


--
-- Name: incident_evidence uq_incident_evidence_incident_version; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_evidence
    ADD CONSTRAINT uq_incident_evidence_incident_version UNIQUE (incident_id, version);


--
-- Name: incidents uq_incidents_incident_number; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incidents
    ADD CONSTRAINT uq_incidents_incident_number UNIQUE (incident_number);


--
-- Name: rca_reports uq_rca_reports_incident_version; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_reports
    ADD CONSTRAINT uq_rca_reports_incident_version UNIQUE (incident_id, version);


--
-- Name: recovery_verifications uq_recovery_verifications_execution_id; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.recovery_verifications
    ADD CONSTRAINT uq_recovery_verifications_execution_id UNIQUE (remediation_execution_id);


--
-- Name: remediation_approvals uq_remediation_approvals_remediation_id; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_approvals
    ADD CONSTRAINT uq_remediation_approvals_remediation_id UNIQUE (remediation_id);


--
-- Name: remediation_executions uq_remediation_executions_remediation_id; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_executions
    ADD CONSTRAINT uq_remediation_executions_remediation_id UNIQUE (remediation_id);
...
--
-- Name: ix_chaos_experiments_scenario_enabled; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_experiments_scenario_enabled ON public.chaos_experiments USING btree (scenario_type, enabled);


--
-- Name: ix_chaos_experiments_service_environment; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_experiments_service_environment ON public.chaos_experiments USING btree (target_service_id, target_environment);


--
-- Name: ix_chaos_observations_run_observed; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_observations_run_observed ON public.chaos_observations USING btree (chaos_run_id, observed_at);


--
-- Name: ix_chaos_observations_type_observed; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_observations_type_observed ON public.chaos_observations USING btree (observation_type, observed_at);


--
-- Name: ix_chaos_runs_experiment_started; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_runs_experiment_started ON public.chaos_runs USING btree (experiment_id, started_at);


--
-- Name: ix_chaos_runs_incident_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_runs_incident_id ON public.chaos_runs USING btree (incident_id);


--
-- Name: ix_chaos_runs_status; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_chaos_runs_status ON public.chaos_runs USING btree (status);
...
--
-- Name: ix_experiment_benchmarks_status_calculated; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_experiment_benchmarks_status_calculated ON public.experiment_benchmarks USING btree (benchmark_status, calculated_at);
...
--
-- Name: ix_incident_evidence_incident_created; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_incident_evidence_incident_created ON public.incident_evidence USING btree (incident_id, created_at);


--
-- Name: ix_incident_evidence_incident_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_incident_evidence_incident_id ON public.incident_evidence USING btree (incident_id);
...
--
-- Name: ix_incidents_current_assignee_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_incidents_current_assignee_id ON public.incidents USING btree (current_assignee_id);


--
-- Name: ix_incidents_deduplication_key; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_incidents_deduplication_key ON public.incidents USING btree (deduplication_key);


--
-- Name: ix_incidents_detected_at; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_incidents_detected_at ON public.incidents USING btree (detected_at);
...
--
-- Name: ix_incidents_primary_service_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_incidents_primary_service_id ON public.incidents USING btree (primary_service_id);
...
--
-- Name: ix_rca_feedback_incident_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_rca_feedback_incident_id ON public.rca_feedback USING btree (incident_id);


--
-- Name: ix_rca_feedback_report_created; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_rca_feedback_report_created ON public.rca_feedback USING btree (report_id, created_at);


--
-- Name: ix_rca_feedback_report_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_rca_feedback_report_id ON public.rca_feedback USING btree (report_id);


--
-- Name: ix_rca_reports_evidence_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_rca_reports_evidence_id ON public.rca_reports USING btree (evidence_id);


--
-- Name: ix_rca_reports_incident_created; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_rca_reports_incident_created ON public.rca_reports USING btree (incident_id, created_at);


--
-- Name: ix_rca_reports_incident_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_rca_reports_incident_id ON public.rca_reports USING btree (incident_id);


--
-- Name: ix_recovery_verifications_remediation_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_recovery_verifications_remediation_id ON public.recovery_verifications USING btree (remediation_id);
...
--
-- Name: ix_remediation_approvals_remediation_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_remediation_approvals_remediation_id ON public.remediation_approvals USING btree (remediation_id);


--
-- Name: ix_remediation_executions_remediation_status; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_remediation_executions_remediation_status ON public.remediation_executions USING btree (remediation_id, execution_status);


--
-- Name: ix_remediation_executions_started_at; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_remediation_executions_started_at ON public.remediation_executions USING btree (started_at);


--
-- Name: ix_remediation_recommendations_incident_created; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_remediation_recommendations_incident_created ON public.remediation_recommendations USING btree (incident_id, created_at);


--
-- Name: ix_remediation_recommendations_service_environment; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_remediation_recommendations_service_environment ON public.remediation_recommendations USING btree (service_id, environment);


--
-- Name: ix_remediation_recommendations_status; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX ix_remediation_recommendations_status ON public.remediation_recommendations USING btree (status);
...
--
-- Name: chaos_experiments chaos_experiments_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_experiments
    ADD CONSTRAINT chaos_experiments_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: chaos_experiments chaos_experiments_target_service_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_experiments
    ADD CONSTRAINT chaos_experiments_target_service_id_fkey FOREIGN KEY (target_service_id) REFERENCES public.services(id) ON DELETE RESTRICT;


--
-- Name: chaos_observations chaos_observations_chaos_run_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_observations
    ADD CONSTRAINT chaos_observations_chaos_run_id_fkey FOREIGN KEY (chaos_run_id) REFERENCES public.chaos_runs(id) ON DELETE CASCADE;


--
-- Name: chaos_runs chaos_runs_experiment_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_experiment_id_fkey FOREIGN KEY (experiment_id) REFERENCES public.chaos_experiments(id) ON DELETE CASCADE;


--
-- Name: chaos_runs chaos_runs_incident_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE SET NULL;


--
-- Name: chaos_runs chaos_runs_rca_report_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_rca_report_id_fkey FOREIGN KEY (rca_report_id) REFERENCES public.rca_reports(id) ON DELETE SET NULL;


--
-- Name: chaos_runs chaos_runs_recovery_verification_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_recovery_verification_id_fkey FOREIGN KEY (recovery_verification_id) REFERENCES public.recovery_verifications(id) ON DELETE SET NULL;


--
-- Name: chaos_runs chaos_runs_remediation_execution_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_remediation_execution_id_fkey FOREIGN KEY (remediation_execution_id) REFERENCES public.remediation_executions(id) ON DELETE SET NULL;


--
-- Name: chaos_runs chaos_runs_remediation_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_remediation_id_fkey FOREIGN KEY (remediation_id) REFERENCES public.remediation_recommendations(id) ON DELETE SET NULL;


--
-- Name: chaos_runs chaos_runs_triggered_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chaos_runs
    ADD CONSTRAINT chaos_runs_triggered_by_fkey FOREIGN KEY (triggered_by) REFERENCES public.users(id) ON DELETE SET NULL;
...
--
-- Name: experiment_benchmarks experiment_benchmarks_chaos_run_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.experiment_benchmarks
    ADD CONSTRAINT experiment_benchmarks_chaos_run_id_fkey FOREIGN KEY (chaos_run_id) REFERENCES public.chaos_runs(id) ON DELETE CASCADE;


--
-- Name: incident_alert_links fk_incident_alert_links_incident_id_incidents; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_alert_links
    ADD CONSTRAINT fk_incident_alert_links_incident_id_incidents FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: incident_alert_links fk_incident_alert_links_reliability_alert_id_reliability_alerts; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_alert_links
    ADD CONSTRAINT fk_incident_alert_links_reliability_alert_id_reliability_alerts FOREIGN KEY (reliability_alert_id) REFERENCES public.reliability_alerts(id) ON DELETE CASCADE;


--
-- Name: incident_assignments fk_incident_assignments_assigned_by_user_id_users; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_assignments
    ADD CONSTRAINT fk_incident_assignments_assigned_by_user_id_users FOREIGN KEY (assigned_by_user_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incident_assignments fk_incident_assignments_assigned_to_user_id_users; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_assignments
    ADD CONSTRAINT fk_incident_assignments_assigned_to_user_id_users FOREIGN KEY (assigned_to_user_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incident_assignments fk_incident_assignments_incident_id_incidents; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_assignments
    ADD CONSTRAINT fk_incident_assignments_incident_id_incidents FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: incident_comments fk_incident_comments_author_user_id_users; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_comments
    ADD CONSTRAINT fk_incident_comments_author_user_id_users FOREIGN KEY (author_user_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incident_comments fk_incident_comments_incident_id_incidents; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_comments
    ADD CONSTRAINT fk_incident_comments_incident_id_incidents FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: incident_metrics fk_incident_metrics_incident_id_incidents; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_metrics
    ADD CONSTRAINT fk_incident_metrics_incident_id_incidents FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: incident_timeline_events fk_incident_timeline_events_actor_user_id_users; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_timeline_events
    ADD CONSTRAINT fk_incident_timeline_events_actor_user_id_users FOREIGN KEY (actor_user_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incident_timeline_events fk_incident_timeline_events_alert_id_reliability_alerts; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_timeline_events
    ADD CONSTRAINT fk_incident_timeline_events_alert_id_reliability_alerts FOREIGN KEY (alert_id) REFERENCES public.reliability_alerts(id) ON DELETE SET NULL;


--
-- Name: incident_timeline_events fk_incident_timeline_events_deployment_id_deployments; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_timeline_events
    ADD CONSTRAINT fk_incident_timeline_events_deployment_id_deployments FOREIGN KEY (deployment_id) REFERENCES public.deployments(id) ON DELETE SET NULL;


--
-- Name: incident_timeline_events fk_incident_timeline_events_incident_id; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_timeline_events
    ADD CONSTRAINT fk_incident_timeline_events_incident_id FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: incidents fk_incidents_created_by_users; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incidents
    ADD CONSTRAINT fk_incidents_created_by_users FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incidents fk_incidents_current_assignee_id_users; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incidents
    ADD CONSTRAINT fk_incidents_current_assignee_id_users FOREIGN KEY (current_assignee_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incidents fk_incidents_primary_service_id_services; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incidents
    ADD CONSTRAINT fk_incidents_primary_service_id_services FOREIGN KEY (primary_service_id) REFERENCES public.services(id) ON DELETE SET NULL;


--
-- Name: incidents fk_incidents_suspected_deployment_id_deployments; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incidents
    ADD CONSTRAINT fk_incidents_suspected_deployment_id_deployments FOREIGN KEY (suspected_deployment_id) REFERENCES public.deployments(id) ON DELETE SET NULL;


--
-- Name: incidents fk_incidents_triggering_alert_id_reliability_alerts; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incidents
    ADD CONSTRAINT fk_incidents_triggering_alert_id_reliability_alerts FOREIGN KEY (triggering_alert_id) REFERENCES public.reliability_alerts(id) ON DELETE SET NULL;


--
-- Name: incident_evidence incident_evidence_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.incident_evidence
    ADD CONSTRAINT incident_evidence_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: incident_evidence incident_evidence_incident_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: rca_feedback rca_feedback_incident_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_feedback
    ADD CONSTRAINT rca_feedback_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: rca_feedback rca_feedback_report_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_feedback
    ADD CONSTRAINT rca_feedback_report_id_fkey FOREIGN KEY (report_id) REFERENCES public.rca_reports(id) ON DELETE CASCADE;


--
-- Name: rca_feedback rca_feedback_reviewer_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_feedback
    ADD CONSTRAINT rca_feedback_reviewer_id_fkey FOREIGN KEY (reviewer_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: rca_reports rca_reports_evidence_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_reports
    ADD CONSTRAINT rca_reports_evidence_id_fkey FOREIGN KEY (evidence_id) REFERENCES public.incident_evidence(id) ON DELETE RESTRICT;


--
-- Name: rca_reports rca_reports_incident_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.rca_reports
    ADD CONSTRAINT rca_reports_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: recovery_verifications recovery_verifications_remediation_execution_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.recovery_verifications
    ADD CONSTRAINT recovery_verifications_remediation_execution_id_fkey FOREIGN KEY (remediation_execution_id) REFERENCES public.remediation_executions(id) ON DELETE CASCADE;


--
-- Name: recovery_verifications recovery_verifications_remediation_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.recovery_verifications
    ADD CONSTRAINT recovery_verifications_remediation_id_fkey FOREIGN KEY (remediation_id) REFERENCES public.remediation_recommendations(id) ON DELETE CASCADE;
...
--
-- Name: remediation_approvals remediation_approvals_approved_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_approvals
    ADD CONSTRAINT remediation_approvals_approved_by_fkey FOREIGN KEY (approved_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: remediation_approvals remediation_approvals_remediation_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_approvals
    ADD CONSTRAINT remediation_approvals_remediation_id_fkey FOREIGN KEY (remediation_id) REFERENCES public.remediation_recommendations(id) ON DELETE CASCADE;


--
-- Name: remediation_executions remediation_executions_remediation_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_executions
    ADD CONSTRAINT remediation_executions_remediation_id_fkey FOREIGN KEY (remediation_id) REFERENCES public.remediation_recommendations(id) ON DELETE CASCADE;


--
-- Name: remediation_recommendations remediation_recommendations_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_recommendations
    ADD CONSTRAINT remediation_recommendations_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: remediation_recommendations remediation_recommendations_incident_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_recommendations
    ADD CONSTRAINT remediation_recommendations_incident_id_fkey FOREIGN KEY (incident_id) REFERENCES public.incidents(id) ON DELETE CASCADE;


--
-- Name: remediation_recommendations remediation_recommendations_service_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.remediation_recommendations
    ADD CONSTRAINT remediation_recommendations_service_id_fkey FOREIGN KEY (service_id) REFERENCES public.services(id) ON DELETE RESTRICT;
...
\unrestrict ucBaALHpKHfNnidNp5lwdjHNe87b0S9pQC8gDBhpymzPS7ZOzAYfvtQvhQ0YSDw
